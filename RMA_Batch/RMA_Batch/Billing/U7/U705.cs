...
public class U705_UPDATE_HDR_DIAG_CD_1 : U705
{
        ...
        : base(Name, Level, true)
    {
        ...
        CLMHDR_DIAG_CD_ALPHA.GetValue += CLMHDR_DIAG_CD_ALPHA_GetValue;
    }
    ...
    private DCharacter CLMHDR_DIAG_CD_ALPHA = new DCharacter("CLMHDR_DIAG_CD_ALPHA", 3);
    private void CLMHDR_DIAG_CD_ALPHA_GetValue(ref string Value)
    {
        try
        {
            Value = QDesign.ASCII(fleF002_SUSPEND_HDR.GetDecimalValue("CLMHDR_DIAG_CD"), 3);
        }
        ...
        try
        {
            ...
            while (fleF002_SUSPEND_DTL.QTPForMissing())
            {
                ...
                while (fleF002_SUSPEND_HDR.QTPForMissing("1"))
                {
                    ...
                    if (Transaction())
                    {
                        fleF002_SUSPEND_HDR.OutPut(OutPutType.Update, null, (QDesign.NULL(CLMHDR_DIAG_CD_ALPHA.Value) == "000" | QDesign.NULL(CLMHDR_DIAG_CD_ALPHA.Value) == QDesign.NULL("   ")) & QDesign.NULL(fleF002_SUSPEND_DTL.GetDecimalValue("CLMDTL_DIAG_CD")) != 0);
