    ...
    public void Run()
    {
        ...
        try
        {
            ...
            while (Sort(fleU030_88_AUTO_ADJ, fleF002_CLMHDR, fleF020_DOCTOR_MSTR, fleICONST_MSTR_REC))
            {
                ...
                m_strWhere = new StringBuilder(" WHERE ");
                m_strWhere.Append(fleF002_ADJ_DTL.ElementOwner("KEY_CLM_TYPE")).Append(" = 'B'");
                m_strWhere.Append(" AND ").Append(fleF002_ADJ_DTL.ElementOwner("KEY_CLM_BATCH_NBR")).Append(" = ");
                m_strWhere.Append(X_CLINIC_BATCH_NBR.Value);
                m_strWhere.Append(" AND ").Append(fleF002_ADJ_DTL.ElementOwner("KEY_CLM_CLAIM_NBR")).Append(" = ");
                m_strWhere.Append(X_CLINIC_BATCH_NBR.Value);

                fleF002_ADJ_DTL.GetData(m_strWhere.ToString());
