        ...
        try
        {
            ...
            while (fleF001_BATCH_CONTROL_FILE.QTPForMissing())
            {
                ...
                while (fleICONST_MSTR_REC.QTPForMissing("1"))
                {
                    ...
                    if (Transaction())
                    {
                        SubFile(ref m_trnTRANS_UPDATE, ref fleU095_DELETE_BATCH, QDesign.NULL(D_DELETE_FLAG.Value) == "Y", SubFileType.Keep, SubFileMode.Append, fleICONST_MSTR_REC, "ICONST_CLINIC_NBR_1_2", D_CUT_OFF_DATE, fleF001_BATCH_CONTROL_FILE);
                        SubFile(ref m_trnTRANS_UPDATE, ref fleU095_RETAIN_BATCH, QDesign.NULL(D_DELETE_FLAG.Value) == "N", SubFileType.Keep, SubFileMode.Append, fleICONST_MSTR_REC, "ICONST_CLINIC_NBR_1_2", D_CUT_OFF_DATE, fleF001_BATCH_CONTROL_FILE);
