    ...
    public void Run()
    {
        ...
        try
        {
            ...
            while (Sort(fleU030_TAPE_145_FILE, fleF020_DOCTOR_MSTR))
            {
                ...
                SubFile(ref m_trnTRANS_UPDATE, ref fleU997_BAD, QDesign.NULL(W_WRONG_FLAG.Value) == "Y", SubFileType.Keep, fleU030_TAPE_145_FILE);




                SubFile(ref m_trnTRANS_UPDATE, ref fleU997_GOOD, QDesign.NULL(W_WRONG_FLAG.Value) == "N", SubFileType.Keep, fleU030_TAPE_145_FILE);




                SubFile(ref m_trnTRANS_UPDATE, ref fleU997_TOTAL, At(W_LAST_REC_FLAG), SubFileType.Keep, W_COUNT, W_RAT_145_AMOUNT_SUB, W_RAT_145_AMT_PAID);
            ...
            while (fleU030_TAPE_RMB_FILE.QTPForMissing())
            {
                ...
                while (fleF020_DOCTOR_MSTR.QTPForMissing("1"))
                {
                    ...
                    if (Transaction())
                    {
                        ...
                        SubFile(ref m_trnTRANS_UPDATE, ref fleU997_BAD, QDesign.NULL(W_WRONG_FLAG.Value) == "Y", SubFileType.Keep, fleU030_TAPE_RMB_FILE);
