...
public class SQL_RELOAD : BaseClassControl
{

    private SQL_RELOAD m_SQL_RELOAD;

    public SQL_RELOAD(string Name, int Level)
        : base(Name, Level)
    {
    ...
    public SQL_RELOAD(string Name, int Level, bool Request)
    ...
    {
        if ((m_SQL_RELOAD != null))
        {
            m_SQL_RELOAD.CloseTransactionObjects();
            m_SQL_RELOAD = null;
        }
    }

    public SQL_RELOAD GetSQL_RELOAD(int Level)
    {
        if (m_SQL_RELOAD == null)
        {
            m_SQL_RELOAD = new SQL_RELOAD("SQL_RELOAD", Level);
        }
        else
        {
            m_SQL_RELOAD.ResetValues();
        }
        return m_SQL_RELOAD;
    }
    ...
    public override bool RunQTP()
    {
        try
        {
            ...
            if (all_tables_found == true)
            {
                ...
                sql.Append(" ");
                sql.Append("        SET @query = N' DROP TABLE IF EXISTS ' + QUOTENAME(cast(@dbname as varchar))  + '.' + QUOTENAME(cast(@schema_name as varchar)) + '.' + QUOTENAME(@table_name); ");
                sql.Append("        EXEC ('Use '+ @backupschema + ';' + @query) ");
                sql.Append(" ");
                sql.Append("        SET @query = 'select * into ' +  QUOTENAME(cast(@dbname as varchar))  + '.' + QUOTENAME(cast(@schema_name as varchar)) + '.' + QUOTENAME(@table_name) + ' from '  + QUOTENAME(@backupschema) + '.' + QUOTENAME(@backup_name + CAST(@earningsperiod as varchar)) + '.' + QUOTENAME(@table_name) + ''; ");
                sql.Append(" ");
                sql.Append("        EXEC ( 'Use '+ @backupschema + ';' + @query) ");
                sql.Append("      END TRY ");
                sql.Append(" ");
