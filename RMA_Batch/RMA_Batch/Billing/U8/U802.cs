...
public class U802_CALC_BATCH_NBR_1 : U802
{
    ...
    {
        this.ScreenType = ScreenTypes.QTP;
        fleADJ_CLAIM_FILE = new SqlFileObject(this, FileTypes.Primary, 0, "SEQUENTIAL", "ADJ_CLAIM_FILE", "", false, false, false, 0, "m_trnTRANS_UPDATE", FileType.SequentialDataBase);
    ...
    public void Run()
    {
        ...
        try
        {
            ...
            while (Sort(fleADJ_CLAIM_FILE))
            {
                ...
                Reset(ref X_COUNT, At(X_CLINIC_NBR));
...
public class U802_CREATE_B_ADJUSTMENT_2 : U802
{
    ...
    private void X_TOT_CLAIM_AR_OMA_GetValue(ref decimal Value)
    {
        ...
        try
        {
            ...
            Value = QDesign.Round(QDesign.Divide((fleF002_CLMHDR.GetDecimalValue("CLMHDR_TOT_CLAIM_AR_OMA") * fleU802_SRT_ADJ.GetDecimalValue("ADJ_AMT_BAL")), fleF002_CLMHDR.GetDecimalValue("CLMHDR_TOT_CLAIM_AR_OHIP")), 0, RoundOptionTypes.Near);
    ...
    private void X_AMT_TECH_BILLED_GetValue(ref decimal Value)
    {
        ...
        try
        {
            ...
            Value = QDesign.Round(QDesign.Divide((fleF002_CLMHDR.GetDecimalValue("CLMHDR_AMT_TECH_BILLED") * fleU802_SRT_ADJ.GetDecimalValue("ADJ_AMT_BAL")), fleF002_CLMHDR.GetDecimalValue("CLMHDR_TOT_CLAIM_AR_OHIP")), 0, RoundOptionTypes.Near);
    ...
    private void fleF001_BATCH_CONTROL_FILE_InitializeItems(bool Fixed)
    {
        ...
        try
        {
            ...
            if (!Fixed)
                fleF001_BATCH_CONTROL_FILE.set_SetValue("BATCTRL_DATE_PERIOD_END", true, fleICONST_MSTR_REC.GetDecimalValue("ICONST_DATE_PERIOD_END_YY").ToString() + fleICONST_MSTR_REC.GetDecimalValue("ICONST_DATE_PERIOD_END_MM").ToString().PadLeft(2, '0') + fleICONST_MSTR_REC.GetDecimalValue("ICONST_DATE_PERIOD_END_DD").ToString().PadLeft(2, '0'));
            if (!Fixed)
    ...
    private void fleF002_ADJ_HDR_InitializeItems(bool Fixed)
    {
        ...
        try
        {
            ...
            if (!Fixed)
                fleF002_ADJ_HDR.set_SetValue("CLMHDR_PAT_ACRONYM6", true, QDesign.Substring(fleU802_SRT_ADJ.GetStringValue("ADJ_PAT_ACRONYM"), 1, 6));
            if (!Fixed)
                fleF002_ADJ_HDR.set_SetValue("CLMHDR_PAT_ACRONYM3", true, QDesign.Substring(fleU802_SRT_ADJ.GetStringValue("ADJ_PAT_ACRONYM"), 7, 3));
            if (!Fixed)
            ...
            if (!Fixed)
                fleF002_ADJ_HDR.set_SetValue("CLMHDR_MSG_NBR", true, QDesign.Substring(QDesign.ASCII(QDesign.SysDate(ref m_cnnQUERY), 8), 1, 2));
            if (!Fixed)
                fleF002_ADJ_HDR.set_SetValue("CLMHDR_REPRINT_FLAG", true, QDesign.Substring(QDesign.ASCII(QDesign.SysDate(ref m_cnnQUERY), 8), 3, 1));
            if (!Fixed)
                fleF002_ADJ_HDR.set_SetValue("CLMHDR_SUB_NBR", true, QDesign.Substring(QDesign.ASCII(QDesign.SysDate(ref m_cnnQUERY), 8), 4, 1));
            if (!Fixed)
                fleF002_ADJ_HDR.set_SetValue("CLMHDR_AUTO_LOGOUT", true, QDesign.Substring(QDesign.ASCII(QDesign.SysDate(ref m_cnnQUERY), 8), 5, 1));
            if (!Fixed)
                fleF002_ADJ_HDR.set_SetValue("CLMHDR_FEE_COMPLEX", true, QDesign.Substring(QDesign.ASCII(QDesign.SysDate(ref m_cnnQUERY), 8), 6, 1));
            if (!Fixed)
                fleF002_ADJ_HDR.set_SetValue("FILLER", true, QDesign.Substring(QDesign.ASCII(QDesign.SysDate(ref m_cnnQUERY), 8), 7, 2));
            if (!Fixed)
    ...
    private void fleF002_ADJ_DTL_InitializeItems(bool Fixed)
    {
        ...
        try
        {
            ...
            if (!Fixed)
                fleF002_ADJ_DTL.set_SetValue("CLMDTL_SV_YY", true, QDesign.Substring(QDesign.ASCII(fleU802_SRT_ADJ.GetDecimalValue("ADJ_SERV_DATE")), 1, 4));
            if (!Fixed)
                fleF002_ADJ_DTL.set_SetValue("CLMDTL_SV_MM", true, QDesign.Substring(QDesign.ASCII(fleU802_SRT_ADJ.GetDecimalValue("ADJ_SERV_DATE")), 5, 2));
            if (!Fixed)
                fleF002_ADJ_DTL.set_SetValue("CLMDTL_SV_DD", true, QDesign.Substring(QDesign.ASCII(fleU802_SRT_ADJ.GetDecimalValue("ADJ_SERV_DATE")), 7, 2));
            if (!Fixed)
                fleF002_ADJ_DTL.set_SetValue("CLMDTL_NBR_SERV", true, 0);
            if (!Fixed)
                fleF002_ADJ_DTL.set_SetValue("CLMDTL_CONSEC_DATES_R", true, 0);
            if (!Fixed)
            ...
            //+ QDesign.ASCII(fleF002_ADJ_HDR.GetDecimalValue("CLMHDR_ORIG_CLAIM_NBR"), 2));
    ...
    private DCharacter KEY_CLM_CLAIM_NBR = new DCharacter("KEY_CLM_CLAIM_NBR", 2);
    private void KEY_CLM_CLAIM_NBR_GetValue(ref string Value)
    {
        try
        {
            Value = QDesign.ASCII(fleF002_ADJ_DTL.GetDecimalValue("KEY_CLM_CLAIM_NBR"), 2);
        }
        ...
        try
        {
            ...
            while (Sort(fleU802_SRT_ADJ, fleF002_CLMHDR, fleF020_DOCTOR_MSTR, fleICONST_MSTR_REC))
            {
               if (X_BATCH_NBR.Value.ToString().StartsWith("99916"))
                ...
                if(updateX_CLINIC_NBR)
                {
                    updateX_CLINIC_NBR = false;
                ...
                if (fleU802_SRT_ADJ.At("X_CLINIC_NBR"))
                    updateX_CLINIC_NBR = true;

                Reset(ref X_BATCH_NBR, fleICONST_MSTR_REC.GetDecimalValue("ICONST_CLINIC_BATCH_NBR") + fleU802_SRT_ADJ.GetDecimalValue("X_BATCH_COUNT"), fleU802_SRT_ADJ.At("X_BATCH_COUNT"));
