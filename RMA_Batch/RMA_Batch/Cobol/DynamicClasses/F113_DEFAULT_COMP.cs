...
namespace RmaDAL
{
    public partial class F113_DEFAULT_COMP : BaseTable
    {
        #region Retrieve

        public ObservableCollection<F113_DEFAULT_COMP> Collection( Guid? rowid,
															string doc_nbr,
                                                            ...
                                                            int skip)
        {
            ...
            if (replaceSearch)
            {
                Reader = CoreReader("[INDEXED].[sp_F113_DEFAULT_COMP_RecordCount]", parameters);
				if (Reader.Read())
					TotalItemCount = (int)Reader[0];
				if (TotalItemCount == 0)
				{
					return new ObservableCollection<F113_DEFAULT_COMP>();
				}

            }

            Reader = CoreReader("[INDEXED].[sp_F113_DEFAULT_COMP_Search]", parameters);
            var collection = new ObservableCollection<F113_DEFAULT_COMP>();

            while (Reader.Read())
            {
                collection.Add(new F113_DEFAULT_COMP
                {
        ...
        public F113_DEFAULT_COMP Class()
        {
        ...
        public ObservableCollection<F113_DEFAULT_COMP> Collection(ObservableCollection<F113_DEFAULT_COMP>
                                                               f113DefaultComp = null)
        {
            if (IsSameSearch() && f113DefaultComp != null)
            {
                return f113DefaultComp;
            }
			...
			Reader = CoreReader("[INDEXED].[sp_F113_DEFAULT_COMP_Match]", parameters);
            var collection = new ObservableCollection<F113_DEFAULT_COMP>();
...
;
        ...
        public bool Delete()
        {
			...
			RowsAffected = CoreExecuteNonQuery("[INDEXED].[sp_F113_DEFAULT_COMP_DeleteRow]", parameters);
		...
		{
		    int RowsAffected = 0;
		    RowsAffected = CoreExecuteNonQuery("[INDEXED].[sp_F113_DEFAULT_COMP_Purge]");
		    CloseConnection();
		    return true;
		}
        ...
        public void Submit()
        {
            ...
            switch (RecordState)
            {
                ...
                case State.Added:
					...
					Reader = CoreReader("[INDEXED].[sp_F113_DEFAULT_COMP_Insert]", parameters);
					if (Reader.Read())
					{
						RowCheckSum = Convert.ToInt32(Reader["RowCheckSum"]);
						ROWID = (Guid) Reader["ROWID"];
                ...
                case State.Modified:
					...
					Reader = CoreReader("[INDEXED].[sp_F113_DEFAULT_COMP_Update]", parameters);
					if (Reader.Read())
					{
						RowCheckSum = Convert.ToInt32(Reader["RowCheckSum"]);
						ROWID = (Guid) Reader["ROWID"];
