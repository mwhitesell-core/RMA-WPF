...
namespace RmaDAL
{
    public partial class F110_COMPENSATION : BaseTable
    {
        #region Retrieve

        public ObservableCollection<F110_COMPENSATION> Collection( Guid? rowid,
															string doc_nbr,
															decimal? ep_nbrmin,
															decimal? ep_nbrmax,
                                                            ...
                                                            int skip)
        {
            ...
            if (replaceSearch)
            {
                Reader = CoreReader("[INDEXED].[sp_F110_COMPENSATION_RecordCount]", parameters);
				if (Reader.Read())
					TotalItemCount = (int)Reader[0];
				if (TotalItemCount == 0)
				{
					return new ObservableCollection<F110_COMPENSATION>();
				}

            }

            Reader = CoreReader("[INDEXED].[sp_F110_COMPENSATION_Search]", parameters);
            var collection = new ObservableCollection<F110_COMPENSATION>();

            while (Reader.Read())
            {
                collection.Add(new F110_COMPENSATION
                {
        ...
        public F110_COMPENSATION Class()
        {
        ...
        public ObservableCollection<F110_COMPENSATION> Collection(ObservableCollection<F110_COMPENSATION>
                                                               f110Compensation = null)
        {
            if (IsSameSearch() && f110Compensation != null)
            {
                return f110Compensation;
            }
			...
			Reader = CoreReader("[INDEXED].[sp_F110_COMPENSATION_Match]", parameters);
            var collection = new ObservableCollection<F110_COMPENSATION>();
...
;
        ...
        public bool Delete()
        {
			...
			RowsAffected = CoreExecuteNonQuery("[INDEXED].[sp_F110_COMPENSATION_DeleteRow]", parameters);
		...
		{
		    int RowsAffected = 0;
		    RowsAffected = CoreExecuteNonQuery("[INDEXED].[sp_F110_COMPENSATION_Purge]");
		    CloseConnection();
		    return true;
		}
        ...
        public void Submit()
        {
            ...
            switch (RecordState)
            {
                ...
                case State.Added:
					...
					Reader = CoreReader("[INDEXED].[sp_F110_COMPENSATION_Insert]", parameters);
					if (Reader.Read())
					{
						RowCheckSum = Convert.ToInt32(Reader["RowCheckSum"]);
						ROWID = (Guid) Reader["ROWID"];
                ...
                case State.Modified:
					...
					Reader = CoreReader("[INDEXED].[sp_F110_COMPENSATION_Update]", parameters);
					if (Reader.Read())
					{
						RowCheckSum = Convert.ToInt32(Reader["RowCheckSum"]);
						ROWID = (Guid) Reader["ROWID"];
