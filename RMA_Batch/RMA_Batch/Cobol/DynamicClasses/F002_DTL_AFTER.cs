...
namespace RmaDAL
{
    public partial class F002_DTL_AFTER : BaseTable
    {
        #region Retrieve

        public ObservableCollection<F002_DTL_AFTER> Collection( Guid? rowid,
                                                            ...
                                                            int skip)
        {
            ...
            if (replaceSearch)
            {
                Reader = CoreReader("[SEQUENTIAL].[sp_F002_DTL_AFTER_RecordCount]", parameters);
				if (Reader.Read())
					TotalItemCount = (int)Reader[0];
				if (TotalItemCount == 0)
				{
					return new ObservableCollection<F002_DTL_AFTER>();
				}

            }

            Reader = CoreReader("[SEQUENTIAL].[sp_F002_DTL_AFTER_Search]", parameters);
            var collection = new ObservableCollection<F002_DTL_AFTER>();

            while (Reader.Read())
            {
                collection.Add(new F002_DTL_AFTER
                {
        ...
        public ObservableCollection<F002_DTL_AFTER> Collection(ObservableCollection<F002_DTL_AFTER>
                                                               f002DtlAfter = null)
        {
            if (IsSameSearch() && f002DtlAfter != null)
            {
                return f002DtlAfter;
            }
			...
			Reader = CoreReader("[SEQUENTIAL].[sp_F002_DTL_AFTER_Match]", parameters);
            var collection = new ObservableCollection<F002_DTL_AFTER>();
...
;
        ...
        public bool Delete()
        {
			...
			RowsAffected = CoreExecuteNonQuery("[SEQUENTIAL].[sp_F002_DTL_AFTER_DeleteRow]", parameters);
		...
		{
		    int RowsAffected = 0;
		    RowsAffected = CoreExecuteNonQuery("[SEQUENTIAL].[sp_F002_DTL_AFTER_Purge]");
		    CloseConnection();
		    return true;
		}
        ...
        public void Submit()
        {
            ...
            switch (RecordState)
            {
                ...
                case State.Added:
					...
					Reader = CoreReader("[SEQUENTIAL].[sp_F002_DTL_AFTER_Insert]", parameters);
					if (Reader.Read())
					{
						RowCheckSum = Convert.ToInt32(Reader["RowCheckSum"]);
						ROWID = (Guid) Reader["ROWID"];
                ...
                case State.Modified:
					...
					Reader = CoreReader("[SEQUENTIAL].[sp_F002_DTL_AFTER_Update]", parameters);
					if (Reader.Read())
					{
						RowCheckSum = Convert.ToInt32(Reader["RowCheckSum"]);
						ROWID = (Guid) Reader["ROWID"];
