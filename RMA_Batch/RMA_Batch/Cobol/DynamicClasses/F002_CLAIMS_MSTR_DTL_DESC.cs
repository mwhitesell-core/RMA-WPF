...
namespace RmaDAL
{
    public partial class F002_CLAIMS_MSTR_DTL_DESC : BaseTable
    {
                                                            ...
                                                            int skip)
        {
            ...
            if (replaceSearch)
            {
                Reader = CoreReader("[INDEXED].[sp_F002_CLAIMS_MSTR_DESC_RecordCount]", parameters);
				if (Reader.Read())
            ...
            Reader = CoreReader("[INDEXED].[sp_F002_CLAIMS_MSTR_DESC_Search]", parameters);
        ...
        public ObservableCollection<F002_CLAIMS_MSTR_DTL_DESC> Collection(ObservableCollection<F002_CLAIMS_MSTR_DTL_DESC>
                                                             f002ClaimsMstrDtlDesc = null, bool isClosedConnection = true, SqlConnection objConn = null)
        {
            if (IsSameSearch() && f002ClaimsMstrDtlDesc != null)
            {
                return f002ClaimsMstrDtlDesc;
           ...
           /* if (IsBlankSearch())
            ...
            {
                Reader = CoreReader("[INDEXED].[sp_F002_CLAIMS_MSTR_DESC_Match]", parameters,objConn);
            } else {
...
;
		...
		{
		    int RowsAffected = 0;
		    RowsAffected = CoreExecuteNonQuery("[INDEXED].[sp_F002_CLAIMS_MSTR_DESC_Purge]");
		    CloseConnection();
		    return true;
		}
