...
namespace RmaDAL
{
    public partial class WEB_HEH_REC : BaseTable
    {
        #region Retrieve

        public ObservableCollection<WEB_HEH_REC> Collection( Guid? rowid,
                                                            ...
                                                            int skip)
        {
            ...
            if (replaceSearch)
            {
                Reader = CoreReader("[SEQUENTIAL].[sp_WEB_HEH_REC_RecordCount]", parameters);
				if (Reader.Read())
					TotalItemCount = (int)Reader[0];
				if (TotalItemCount == 0)
				{
					return new ObservableCollection<WEB_HEH_REC>();
				}

            }

            Reader = CoreReader("[SEQUENTIAL].[sp_WEB_HEH_REC_Search]", parameters);
            var collection = new ObservableCollection<WEB_HEH_REC>();

            while (Reader.Read())
            {
                collection.Add(new WEB_HEH_REC
                {
        ...
        public ObservableCollection<WEB_HEH_REC> Collection(ObservableCollection<WEB_HEH_REC>
                                                               webHehRec = null)
        {
            if (IsSameSearch() && webHehRec != null)
            {
                return webHehRec;
            }
			...
			Reader = CoreReader("[SEQUENTIAL].[sp_WEB_HEH_REC_Match]", parameters);
            var collection = new ObservableCollection<WEB_HEH_REC>();
...
;
        ...
        public bool Delete()
        {
			...
			RowsAffected = CoreExecuteNonQuery("[SEQUENTIAL].[sp_WEB_HEH_REC_DeleteRow]", parameters);
		...
		{
		    int RowsAffected = 0;
		    RowsAffected = CoreExecuteNonQuery("[SEQUENTIAL].[sp_WEB_HEH_REC_Purge]");
		    CloseConnection();
		    return true;
		}
        ...
        public void Submit()
        {
            ...
            switch (RecordState)
            {
                ...
                case State.Added:
					...
					Reader = CoreReader("[SEQUENTIAL].[sp_WEB_HEH_REC_Insert]", parameters);
					if (Reader.Read())
					{
						RowCheckSum = Convert.ToInt32(Reader["RowCheckSum"]);
						ROWID = (Guid) Reader["ROWID"];
                ...
                case State.Modified:
					...
					Reader = CoreReader("[SEQUENTIAL].[sp_WEB_HEH_REC_Update]", parameters);
					if (Reader.Read())
					{
						RowCheckSum = Convert.ToInt32(Reader["RowCheckSum"]);
						ROWID = (Guid) Reader["ROWID"];
