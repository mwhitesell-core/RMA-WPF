...
namespace RmaDAL
{
    public partial class F020C_DOC_CLINIC_NEXT_BATCH_NBR : BaseTable
    {
        #region Retrieve

        public ObservableCollection<F020C_DOC_CLINIC_NEXT_BATCH_NBR> Collection( Guid? rowid,
															string doc_nbr,
                                                            ...
                                                            int skip)
        {
            ...
            if (replaceSearch)
            {
                Reader = CoreReader("[INDEXED].[sp_F020C_DOC_CLINIC_NEXT_BATCH_NBR_RecordCount]", parameters);
				if (Reader.Read())
					TotalItemCount = (int)Reader[0];
				if (TotalItemCount == 0)
				{
					return new ObservableCollection<F020C_DOC_CLINIC_NEXT_BATCH_NBR>();
				}

            }

            Reader = CoreReader("[INDEXED].[sp_F020C_DOC_CLINIC_NEXT_BATCH_NBR_Search]", parameters);
            var collection = new ObservableCollection<F020C_DOC_CLINIC_NEXT_BATCH_NBR>();

            while (Reader.Read())
            {
                collection.Add(new F020C_DOC_CLINIC_NEXT_BATCH_NBR
                {
        ...
        public F020C_DOC_CLINIC_NEXT_BATCH_NBR Class()
        {
        ...
        public ObservableCollection<F020C_DOC_CLINIC_NEXT_BATCH_NBR> Collection(ObservableCollection<F020C_DOC_CLINIC_NEXT_BATCH_NBR>
                                                               f020cDocClinicNextBatchNbr = null)
        {
            if (IsSameSearch() && f020cDocClinicNextBatchNbr != null)
            {
                return f020cDocClinicNextBatchNbr;
            }
			...
			Reader = CoreReader("[INDEXED].[sp_F020C_DOC_CLINIC_NEXT_BATCH_NBR_Match]", parameters);
            var collection = new ObservableCollection<F020C_DOC_CLINIC_NEXT_BATCH_NBR>();
...
;
        ...
        public bool Delete()
        {
			...
			RowsAffected = CoreExecuteNonQuery("[INDEXED].[sp_F020C_DOC_CLINIC_NEXT_BATCH_NBR_DeleteRow]", parameters);
		...
		{
		    int RowsAffected = 0;
		    RowsAffected = CoreExecuteNonQuery("[INDEXED].[sp_F020C_DOC_CLINIC_NEXT_BATCH_NBR_Purge]");
		    CloseConnection();
		    return true;
		}
        ...
        public void Submit()
        {
            ...
            switch (RecordState)
            {
                ...
                case State.Added:
					...
					Reader = CoreReader("[INDEXED].[sp_F020C_DOC_CLINIC_NEXT_BATCH_NBR_Insert]", parameters);
					if (Reader.Read())
					{
						RowCheckSum = Convert.ToInt32(Reader["RowCheckSum"]);
						ROWID = (Guid) Reader["ROWID"];
                ...
                case State.Modified:
					...
					Reader = CoreReader("[INDEXED].[sp_F020C_DOC_CLINIC_NEXT_BATCH_NBR_Update]", parameters);
					if (Reader.Read())
					{
						RowCheckSum = Convert.ToInt32(Reader["RowCheckSum"]);
						ROWID = (Guid) Reader["ROWID"];
