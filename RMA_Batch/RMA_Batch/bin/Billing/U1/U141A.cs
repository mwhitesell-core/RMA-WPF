...
fleF040_DTL_DOC.InitializeItems += fleF040_DTL_DOC_AutomaticItemInitialization;
	...
	private void VALID_CLINIC_GetValue(ref string Value)
	{
		...
		try {
			string CurrentValue = string.Empty;
			if ((QDesign.NULL(fleMISC_PAYMENT_FILE.GetDecimalValue("CLINIC_NBR")) == QDesign.NULL(fleF020_DOCTOR_MSTR.GetDecimalValue("DOC_CLINIC_NBR")) | QDesign.NULL(fleMISC_PAYMENT_FILE.GetDecimalValue("CLINIC_NBR")) == QDesign.NULL(fleF020_DOCTOR_MSTR.GetDecimalValue("DOC_CLINIC_NBR_2")) | QDesign.NULL(fleMISC_PAYMENT_FILE.GetDecimalValue("CLINIC_NBR")) == QDesign.NULL(fleF020_DOCTOR_MSTR.GetDecimalValue("DOC_CLINIC_NBR_3")) | QDesign.NULL(fleMISC_PAYMENT_FILE.GetDecimalValue("CLINIC_NBR")) == QDesign.NULL(fleF020_DOCTOR_MSTR.GetDecimalValue("DOC_CLINIC_NBR_4")) | QDesign.NULL(fleMISC_PAYMENT_FILE.GetDecimalValue("CLINIC_NBR")) == QDesign.NULL(fleF020_DOCTOR_MSTR.GetDecimalValue("DOC_CLINIC_NBR_5")) | QDesign.NULL(fleMISC_PAYMENT_FILE.GetDecimalValue("CLINIC_NBR")) == QDesign.NULL(fleF020_DOCTOR_MSTR.GetDecimalValue("DOC_CLINIC_NBR_6")))) {
				CurrentValue = "Y";
			} else {
				CurrentValue = "2";
			}
	...
	private void TERM_DAYS_GetValue(ref decimal Value)
	{
		...
		try {
			Value = QDesign.Days(QDesign.SysDate(ref m_cnnQUERY)) - QDesign.Days(fleF020_DOCTOR_MSTR.GetNumericDateValue("DOC_DATE_FAC_TERM"));
	...
	private void VALID_TERMINATE_GetValue(ref string Value)
	{
		...
		try {
			string CurrentValue = string.Empty;
			if (QDesign.NULL(fleF020_DOCTOR_MSTR.GetNumericDateValue("DOC_DATE_FAC_TERM")) != 0 & TERM_DAYS.Value <= 180 & QDesign.NULL(UP_BYPASS.Value) != "BYPASS") {
				CurrentValue = "W";
			} else if (QDesign.NULL(fleF020_DOCTOR_MSTR.GetNumericDateValue("DOC_DATE_FAC_TERM")) != 0 & QDesign.NULL(TERM_DAYS.Value) > 180 & QDesign.NULL(UP_BYPASS.Value) != "BYPASS") {
				CurrentValue = "E";
			} else {
				CurrentValue = "Y";
			}
			...
			while (fleMISC_PAYMENT_FILE.QTPForMissing()) {
				...
				while (fleF020_DOCTOR_MSTR.QTPForMissing("1")) {
					...
					while (fleF040_OMA_FEE_MSTR.QTPForMissing("2")) {
						...
						m_strWhere.Append(Common.StringToField((fleMISC_PAYMENT_FILE.GetStringValue("CLMDTL_OMA_CD")).PadRight(4).Substring(1, 1)));
						

						fleF040_OMA_FEE_MSTR.GetData(m_strWhere.ToString(), GetDataOptions.IsOptional);
						// --> End GET F040_OMA_FEE_MSTR <--

						while (fleF040_DTL.QTPForMissing("3")) {
							// --> GET F040_DTL <--
							m_strWhere = new StringBuilder(" WHERE ");
							...
							while (fleF040_DTL_DOC.QTPForMissing("4")) {
								// --> GET F040_DTL_DOC <--
								m_strWhere = new StringBuilder(" WHERE ");
