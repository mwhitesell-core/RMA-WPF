...
public class U020B_CREATE_TAPE_3 : U020B
{
    ...
    private void X_ACCOUNTING_NBR_GetValue(ref string Value)
    {
        ...
        try
        {
            Value= QDesign.Substring(fleU020B.GetStringValue("CLMHDR_CLAIM_ID"), 3, 8);
		...
		try {
			...
			while (Sort(fleU020B, fleTMP_COUNTERS_ALPHA)) {
				...
				while (fleF099_GROUP_CLAIM_MSTR.QTPForMissing()) {
					...
					m_strWhere = new StringBuilder(" WHERE ");
					m_strWhere.Append(" ").Append(fleF099_GROUP_CLAIM_MSTR.ElementOwner("CLMHDR_BATCH_NBR")).Append(" = ");
