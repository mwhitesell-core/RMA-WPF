...
namespace RMA_DATA
{
    public class R093A : BaseRDLClass
    {
        protected const string REPORT_NAME = "R093A";
        protected const bool REPORT_HAS_PARAMETERS = false;

        private Reader rdrU093_RETAIN_BATCH = new Reader();
        ...
        private void Access_U093_RETAIN_BATCH()
        {
            ...
            strSQL.Append("FROM TEMPORARYDATA.U093_RETAIN_BATCH ");

            strSQL.Append(Choose());
            strSQL.Append(" ORDER BY BATCTRL_BATCH_NBR");
            rdrU093_RETAIN_BATCH.GetDataTable = TextHelper.ExecuteDataTable(strSQL.ToString(), ReportFunctions.m_strFlatFilePath, ReportFunctions.TextFiles);
            strSQL = null;
        }
        ...
        private void Link_ICONST_MSTR_REC()
        {
            ...
            strSQL.Append("WHERE ");
            strSQL.Append("ICONST_CLINIC_NBR_1_2 = ").Append(Common.StringToField(QDesign.Substring(rdrU093_RETAIN_BATCH.GetString("BATCTRL_BATCH_NBR"), 1, 2)));
        ...
        private decimal R_BATCTRL_CALC_AR_DUE()
        {
            ...
            try
            {
                if (rdrU093_RETAIN_BATCH.GetString("BATCTRL_BATCH_TYPE") == "P" && rdrU093_RETAIN_BATCH.GetString("BATCTRL_ADJ_CD") == "C")
                {
                    decReturnValue = 0 - rdrU093_RETAIN_BATCH.GetNumber("BATCTRL_CALC_AR_DUE");
                }
                else
                {
                    decReturnValue = rdrU093_RETAIN_BATCH.GetNumber("BATCTRL_CALC_AR_DUE");
                }
            }
        ...
        private decimal R_BATCTRL_CALC_TOT_REV()
        {
            ...
            try
            {
                if (rdrU093_RETAIN_BATCH.GetString("BATCTRL_BATCH_TYPE") == "P" && rdrU093_RETAIN_BATCH.GetString("BATCTRL_ADJ_CD") == "C")
                {
                    decReturnValue = 0 - rdrU093_RETAIN_BATCH.GetNumber("BATCTRL_CALC_TOT_REV");
                }
                else
                {
                    decReturnValue = rdrU093_RETAIN_BATCH.GetNumber("BATCTRL_CALC_TOT_REV");
                }
            }
        ...
        private decimal R_BATCTRL_MANUAL_PAY_TOT()
        {
            ...
            try
            {
                if (rdrU093_RETAIN_BATCH.GetString("BATCTRL_BATCH_TYPE") == "P" && rdrU093_RETAIN_BATCH.GetString("BATCTRL_ADJ_CD") == "C")
                {
                    decReturnValue = 0 - rdrU093_RETAIN_BATCH.GetNumber("BATCTRL_MANUAL_PAY_TOT");
                }
                else
                {
                    decReturnValue = rdrU093_RETAIN_BATCH.GetNumber("BATCTRL_MANUAL_PAY_TOT");
                }
            }
        ...
        private string D_BATCTRL_ADJ_CD()
        {
            ...
            try
            {
                if (rdrU093_RETAIN_BATCH.GetString("BATCTRL_BATCH_TYPE") == "C")
                {
                    strReturnValue = QDesign.ASCII(rdrU093_RETAIN_BATCH.GetNumber("BATCTRL_AGENT_CD"));
                }
                else
                {
                    strReturnValue = rdrU093_RETAIN_BATCH.GetString("BATCTRL_ADJ_CD");
                }
            }
        ...
        public override void DeclareReportControls()
        {
            try
            {
                ...
                AddControl(ReportSection.SUMMARY, "TEMPORARYDATA.U093_RETAIN_BATCH.D_CUT_OFF_DATE", DataTypes.Numeric, 8);
                ...
                AddControl(ReportSection.SUMMARY, "TEMPORARYDATA.U093_RETAIN_BATCH.BATCTRL_BATCH_TYPE", DataTypes.Character, 1);
                AddControl(ReportSection.SUMMARY, "D_BATCTRL_ADJ_CD", DataTypes.Character, 1);
                AddControl(ReportSection.SUMMARY, "TEMPORARYDATA.U093_RETAIN_BATCH.BATCTRL_CYCLE_NBR", DataTypes.Numeric, 3);
        ...
        public override string ReturnControlValue(string strControl, int intSize)
        {
            switch (strControl)
            {
                case "TEMPORARYDATA.U093_RETAIN_BATCH.BATCTRL_BATCH_NBR":
                    return Common.StringToField(rdrU093_RETAIN_BATCH.GetString("BATCTRL_BATCH_NBR"));
                ...
                case "TEMPORARYDATA.U093_RETAIN_BATCH.D_CUT_OFF_DATE":
                    return rdrU093_RETAIN_BATCH.GetNumber("D_CUT_OFF_DATE").ToString();
                ...
                case "TEMPORARYDATA.U093_RETAIN_BATCH.BATCTRL_BATCH_TYPE":
                    return Common.StringToField(rdrU093_RETAIN_BATCH.GetString("BATCTRL_BATCH_TYPE"));

                case "D_BATCTRL_ADJ_CD":
                    return Common.StringToField(D_BATCTRL_ADJ_CD(), intSize);

                case "TEMPORARYDATA.U093_RETAIN_BATCH.BATCTRL_CYCLE_NBR":
                    return rdrU093_RETAIN_BATCH.GetNumber("BATCTRL_CYCLE_NBR").ToString();
        ...
        public override void AccessData()
        {
            try
            {
                ...
                Access_U093_RETAIN_BATCH();
                while (rdrU093_RETAIN_BATCH.Read())
                {
        ...
        {
            if (!(rdrU093_RETAIN_BATCH == null))
            {
                rdrU093_RETAIN_BATCH.Close();
                rdrU093_RETAIN_BATCH = null;
            }
