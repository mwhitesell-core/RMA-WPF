...
namespace RMA_DATA
{
    public class R121E : BaseRDLClass
    {
        #region " Renaissance Declarations (Variables, DataReaders, Parameters and Use Files) "

        protected const string REPORT_NAME = "R121E";
        ...
        private void Access_Data()
        {
            ...
            // GW2018. April 30. Changed. strSQL.Append("CONCAT(ytd.COMP_CODE_GROUP, CASE WHEN len(right(reporting_seq,2)) < 2 THEN REPLICATE('0', len(right(reporting_seq,2))) + right(reporting_seq,2) ELSE right(reporting_seq,2) END, ytd.COMP_CODE) x_key1 ");
            ...
            strSQL.Append("WHERE ytd.REC_TYPE = 'A' AND dept.DEPT_COMPANY = 3) AS alldata ");
        ...
        public override string ReturnControlValue(string strControl, int intSize)
        {
            switch (strControl)
            {
                case "INDEXED.F123_COMPANY_MSTR.COMPANY_NAME":
                    return Common.StringToField(rdrDATA.GetString("COMPANY_NAME").PadRight(24, ' '));
