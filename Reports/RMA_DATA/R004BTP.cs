...
// 97/JAN/19  J.C.      S149 - ADD DEPT-NBR TO FILE LINKAGE
// 03/dec/17  A.A.      alpha doctor nbr
// LINK ( B  , CLMHDR-BATCH-NBR) TO KEY-CLM-TYPE,    &
// jc
...
namespace RMA_DATA
{
    public class R004BTP : BaseRDLClass
    {
        #region " Renaissance Declarations (Variables, DataReaders, Parameters and Use Files) "

        protected const string REPORT_NAME = "R004BTP";
        ...
        public override void DeclareReportControls()
        {
            try
            {
                AddControl(ReportSection.PAGE_HEADING, "TEMPORARYDATA.R004ATP.ICONST_CLINIC_NBR_1_2", DataTypes.Numeric, 3);
                AddControl(ReportSection.PAGE_HEADING, "TEMPORARYDATA.R004ATP.ICONST_DATE_PERIOD_END", DataTypes.Numeric, 9);
                ...
                AddControl(ReportSection.PAGE_HEADING, "TEMPORARYDATA.R004ATP.CLMHDR_DOC_DEPT", DataTypes.Numeric, 3);
        ...
        public override string ReturnControlValue(string strControl, int intSize)
        {
            switch (strControl)
            {
                ...
                case "X_NBR_SVCS":
                    return X_NBR_SVCS().ToString().ToString();

                case "X_CLMHDR_REFERENCE":
                    return Common.StringToField(X_CLMHDR_REFERENCE());

                case "X_AR_OHIP_FEE":
                    return X_AR_OHIP_FEE().ToString().ToString();

                case "X_AR_TECH_FEE":
                    return X_AR_TECH_FEE().ToString().ToString();

                case "X_AR_PROF_FEE":
                    return X_AR_PROF_FEE().ToString().ToString();

                case "X_AR_OHIP_ADJ":
                    return X_AR_OHIP_ADJ().ToString().ToString();

                case "X_AR_TECH_ADJ":
                    return X_AR_TECH_ADJ().ToString().ToString();

                case "X_AR_PROF_ADJ":
                    return X_AR_PROF_ADJ().ToString().ToString();

                case "X_AR_OHIP_TOTAL":
                    return X_AR_OHIP_TOTAL().ToString().ToString();

                case "X_AR_TECH_TOTAL":
                    return X_AR_TECH_TOTAL().ToString().ToString();

                case "X_AR_PROF_TOTAL":
                    return X_AR_PROF_TOTAL().ToString().ToString();

                case "X_REV_OHIP_TOTAL":
                    return X_REV_OHIP_TOTAL().ToString().ToString();

                case "X_REV_TECH_TOTAL":
                    return X_REV_TECH_TOTAL().ToString().ToString();

                case "X_REV_PROF_TOTAL":
                    return X_REV_PROF_TOTAL().ToString().ToString();
