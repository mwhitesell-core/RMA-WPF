...
// DOC: SORT BY CLINIC/AGENT/LOCATION/DOCTOR
// DOC: RUN FOR: MUMC DIAGNOSTICS
// PROGRAM PURPOSE : CASH ANALYSIS BY LOCATION (DETAIL REPORT)
// R005TP.CB CONVERSTION TO POWERHOUSE R005ATP.QZS
// APPEND SUMMARY REPORTS R005BTP.TXT AND R005CTP.TXT
// TO END OF R005ATP.TXT
// DATE       BY WHO       DESCRIPTION
// 2007/Mar   YASEMIN      r005atp 
...
namespace RMA_DATA
{
    public class R005ATP_70 : BaseRDLClass
    {
        #region " Renaissance Declarations (Variables, DataReaders, Parameters and Use Files) "

        protected const string REPORT_NAME = "R005ATP_70";
        ...
        public DataSet GetDataSet(string strConnection, string[] arrParameters, string strReportAssembly, bool blnDebug)
        {
            try
            {
                ...
                Sort = "DOCASHTP_CLINIC_NBR ASC, DOCASHTP_AGENT_CD ASC, DOCASHTP_LOC_CD ASC, DOCASHTP_DOC_NBR ASC";
        ...
        private void Link_F030_LOCATIONS_MSTR()
        {
            ...
            strSQL.Append("FROM [101C].INDEXED.F030_LOCATIONS_MSTR ");
            strSQL.Append("WHERE ");
        ...
        private string X_MONTH()
        {
            ...
            try
            {
                if (QDesign.NULL(X_MTH()) == QDesign.NULL("01"))
                {
                    strReturnValue = "JANUARY";
                }
                else if (QDesign.NULL(X_MTH()) == QDesign.NULL("02"))
                {
                    strReturnValue = "FEBRUARY";
                }
                else if (QDesign.NULL(X_MTH()) == QDesign.NULL("03"))
                {
                    strReturnValue = "MARCH";
                }
                else if (QDesign.NULL(X_MTH()) == QDesign.NULL("04"))
                {
                    strReturnValue = "APRIL";
                }
                else if (QDesign.NULL(X_MTH()) == QDesign.NULL("05"))
                {
                    strReturnValue = "MAY";
                }
                else if (QDesign.NULL(X_MTH()) == QDesign.NULL("06") & QDesign.NULL(X_DAY()).CompareTo(QDesign.NULL("30")) < 0)
                {
                    strReturnValue = "JUNE";
                }
                else if (QDesign.NULL(X_MTH()) == QDesign.NULL("06") & QDesign.NULL(X_DAY()) == QDesign.NULL("30"))
                {
                    strReturnValue = "YEAREND";
                }
                else if (QDesign.NULL(X_MTH()) == QDesign.NULL("07"))
                {
                    strReturnValue = "JULY";
                }
                else if (QDesign.NULL(X_MTH()) == QDesign.NULL("08"))
                {
                    strReturnValue = "AUGUST";
                }
                else if (QDesign.NULL(X_MTH()) == QDesign.NULL("09"))
                {
                    strReturnValue = "SEPTEMBER";
                }
                else if (QDesign.NULL(X_MTH()) == QDesign.NULL("10"))
                {
                    strReturnValue = "OCTOBER";
                }
                else if (QDesign.NULL(X_MTH()) == QDesign.NULL("11"))
                {
                    strReturnValue = "NOVEMBER";
                }
                else if (QDesign.NULL(X_MTH()) == QDesign.NULL("12"))
                {
        ...
        public override void DeclareReportControls()
        {
            try
            {
                ...
                AddControl(ReportSection.PAGE_HEADING, "INDEXED.F051TP_DOC_CASH_MSTR.DOCASHTP_LOC_CD", DataTypes.Character, 4);
                AddControl(ReportSection.PAGE_HEADING, "INDEXED.F030_LOCATIONS_MSTR.LOC_NAME", DataTypes.Character, 24);
        ...
        //# Do not delete, modify or move it.  Updated: 10/24/2017 11:01:15 AM
        ...
        public override string ReturnControlValue(string strControl, int intSize)
        {
            switch (strControl)
            {
                ...
                case "X_DATE":
                    return Common.StringToField(X_DATE());
                ...
                case "X_NAME":
                    return Common.StringToField(X_NAME());
