...
// 2009/jul/07 MC2      - include f074-afp-group-sequence-mstr in the access
// - select based on the flag instead of hard code the group
// - cosmetic change on the final footing for alignment and spacing
// - display `10/60` if incoming percentage is 0
// 60
...
namespace RMA_DATA
{
    public class R140_A3 : BaseRDLClass
    {
        #region " Renaissance Declarations (Variables, DataReaders, Parameters and Use Files) "

        protected const string REPORT_NAME = "R140_A3";
        ...
        //# Do not delete, modify or move it.  Updated: 11/14/2017 1:23:30 PM
        ...
        public DataSet GetDataSet(string strConnection, string[] arrParameters, string strReportAssembly, bool blnDebug)
        {
            try
            {
                ...
                Sort = "AFP_REPORTING_MTH ASC, DOC_AFP_PAYM_GROUP ASC";
        ...
        private void Link_F074_AFP_GROUP_SEQUENCE_MSTR()
        {
            ...
            strSQL.Append("WHERE ");
            strSQL.Append("DOC_AFP_PAYM_GROUP = ").Append(Common.StringToField(rdrF074_AFP_GROUP_MSTR.GetString("DOC_AFP_PAYM_GROUP")));
        ...
        public override void DeclareReportControls()
        {
            try
            {
                ...
                AddControl(ReportSection.FINAL_FOOTING, "RMA_INC_TOTAL", DataTypes.Numeric, 10);
                AddControl(ReportSection.FINAL_FOOTING, "OTHER_TOTAL", DataTypes.Numeric, 10);
                AddControl(ReportSection.FINAL_FOOTING, "RMA_INC_SUB_TOTAL", DataTypes.Numeric, 10);
                AddControl(ReportSection.FINAL_FOOTING, "RMA_TOTAL", DataTypes.Numeric, 10);
                AddControl(ReportSection.FINAL_FOOTING, "GRAND_TOTAL", DataTypes.Numeric, 10);
            }
        ...
        public override string ReturnControlValue(string strControl, int intSize)
        {
            switch (strControl)
            {
                ...
                case "TEMPORARYDATA.R140_A1.X_PAYMENT_AMT":
                    return rdrR140_A1.GetNumber("X_PAYMENT_AMT").ToString().PadLeft(10, ' ');

                case "TEMPORARYDATA.R140_A1.X_SUBMISSION_AMT":
                    return rdrR140_A1.GetNumber("X_SUBMISSION_AMT").ToString().PadLeft(10, ' ');

                case "X_PAYMENT_GRAND_TOTAL":
                    return X_PAYMENT_GRAND_TOTAL().ToString().PadLeft(6, ' ');

                case "SOLO_TOTAL":
                        return SOLO_TOTAL().ToString().ToString().PadLeft(10, ' ');

                case "RMA_INC_TOTAL":
                    return RMA_INC_TOTAL().ToString().PadLeft(10, ' ');

                case "OTHER_TOTAL":
                    return OTHER_TOTAL().ToString().PadLeft(10, ' ');

                case "RMA_INC_SUB_TOTAL":
                    return RMA_INC_SUB_TOTAL().ToString().PadLeft(10, ' ');

                case "RMA_TOTAL":
                    return RMA_TOTAL().ToString().PadLeft(10, ' ');

                case "GRAND_TOTAL":
                    return GRAND_TOTAL().ToString().PadLeft(10, ' ');
            ...
            try
            {
                ...
                while (rdrR140_A1.Read())
                {
                    ...
                    while (rdrF074_AFP_GROUP_MSTR.Read())
                    {
                        ...
                        Link_TMP_COUNTERS();
                        while ((rdrTMP_COUNTERS.Read() | rdrF074_AFP_GROUP_SEQUENCE_MSTR.Read()))
                        {
                            WriteData();
                        }
