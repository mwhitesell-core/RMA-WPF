...
namespace RMA_DATA
{
    public class MP_R119A : BaseRDLClass
    {
        ...
        private void Access_Data()
        {
            ...
            strSQL.Append("inner join INDEXED.F112_PYCDCEILINGS py on py.EP_NBR = constmstr.CURRENT_EP_NBR and py.DOC_NBR = ytd.DOC_NBR ");
            strSQL.Append("WHERE ytd.REC_TYPE = 'A' ");
            strSQL.Append("and py.DOC_PAY_CODE != 7 ");
            strSQL.Append("ORDER BY mstr.DOC_DEPT, mstr.DOC_FULL_PART_IND");
            strSQL.Append(", ytd.comp_code_group + right(reporting_seq, 2) + ytd.comp_code");
        ...
        private string DOC_FULL_PART_INDDOC_DEPT()
        {
            string strReturnValue = string.Empty;

            try
            {
                strReturnValue = rdrDATA.GetString("DOC_FULL_PART_IND") + rdrDATA.GetNumber("DOC_DEPT").ToString();
            }
        ...
        public override void DeclareReportControls()
        {
            try
            {
                ...
                AddControl(ReportSection.REPORT, "DOC_FULL_PART_INDDOC_DEPT", DataTypes.Character, 3);
        ...
        public override string ReturnControlValue(string strControl, int intSize)
        {
            switch (strControl)
            {
                ...
                case "DOC_FULL_PART_INDDOC_DEPT":
                    return Common.StringToField(DOC_FULL_PART_INDDOC_DEPT(), intSize);
