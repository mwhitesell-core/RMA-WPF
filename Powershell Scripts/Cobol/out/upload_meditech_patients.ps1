...
Get-Content meditech_patient_file | awk++ $cmd\reformat_meditech_patient_file_2.awk  > meditech_patient_file.u011
...
$now = "$(Get-Date -uformat `"%Y_%m_%d`")_$(Get-Date -uformat `"%T`")"

echo ""
echo "Reports ru011a\b\c renamed for backup purposes ..."
Move-Item ru011a ru011a.$now
Move-Item ru011b ru011b.$now
Move-Item ru011c ru011c.$now

echo ""
echo "Run is successful - renaming input and output iles to indicate processed ..."
echo ""
Move-Item meditech_patient_file.u011 meditech_patient_file.u011.${1}
Move-Item meditech_patient_file.out meditech_patient_file.out.${1}
