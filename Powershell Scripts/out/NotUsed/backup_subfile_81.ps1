#-------------------------------------------------------------------------------
# File 'backup_subfile_81.ps1'
# Converted to PowerShell by CORE Migration on 2017-12-06 16:09:57
# Original file name was 'backup_subfile_81'
#-------------------------------------------------------------------------------

clear
echo "BACKUP OF MONTHLY CLAIM SUBFILE - PLACE APPROPRIATE TAPE ONLINE"
echo ""
 if (Test-Path claims_subfile_81_jun.sf)
 {
             if (Test-Path claims_subfile_81_yr.sf)
             {
                            &$env:cmd\call_error_duplicate_files jun yr
            } else {
               if (Test-Path claims_subfile_81_jul.sf)
               {
                                &$env:cmd\call_error_duplicate_files jun jul
              } else {
                 if (Test-Path claims_subfile_81_aug.sf)
                 {
                                    &$env:cmd\call_error_duplicate_files jun aug
                } else {
                   if (Test-Path claims_subfile_81_sep.sf)
                   {
                                        &$env:cmd\call_error_duplicate_files jun sep
                  } else {
                     if (Test-Path claims_subfile_81_oct.sf)
                     {
                                            &$env:cmd\call_error_duplicate_files jun oct
                    } else {
                       if (Test-Path claims_subfile_81_nov.sf)
                       {
                                                &$env:cmd\call_error_duplicate_files jun nov
                      } else {
                         if (Test-Path claims_subfile_81_dec.sf)
                         {
                                                    &$env:cmd\call_error_duplicate_files jun dec
                        } else {
                                                  &$env:cmd\call_backup_mthly_sub81 jun 0
                        }
                }
                }
                }
...
#  else
#   if [ -f claims_subfile_81_yr.sf ]
#         if [ -f claims_subfile_81_jul.sf ]
#          $cmd/call_error_duplicate_files yr jul
#        else
#           if [ -f claims_subfile_81_aug.sf ]
#            $cmd/call_error_duplicate_files yr aug
#          else
#             if [ -f claims_subfile_81_sep.sf ]
#              $cmd/call_error_duplicate_files yr sep
#            else
#               if [ -f claims_subfile_81_oct.sf ]
#                $cmd/call_error_duplicate_files yr oct
#              else
#                 if [ -f claims_subfile_81_nov.sf ]
#                  $cmd/call_error_duplicate_files yr nov
#                else
#                   if [ -f claims_subfile_81_dec.sf ]
#                    $cmd/call_error_duplicate_files yr dec
#                  else
#                          $cmd/call_backup_mthly_sub81 yr 0
#                   fi
#                  fi
#                  fi
#            fi
...
#        fi
                       ...
                       if (Test-Path claims_subfile_81_nov.sf)
                       {
                         ...
                         if (Test-Path claims_subfile_81_dec.sf)
                         {
                                                    &$env:cmd\call_error_duplicate_files jul dec
                                                      ...
                                                      &$env:cmd\call_backup_mthly_sub81 jul 1
                        }
                }
                }
                }
                       ...
                       if (Test-Path claims_subfile_81_nov.sf)
                       {
                         ...
                         if (Test-Path claims_subfile_81_dec.sf)
                         {
                                                    &$env:cmd\call_error_duplicate_files aug dec
                                                      ...
                                                      &$env:cmd\call_backup_mthly_sub81 aug 2
                        }
                }
                }
                }
                     ...
                     if (Test-Path claims_subfile_81_oct.sf)
                     {
                       ...
                       if (Test-Path claims_subfile_81_nov.sf)
                       {
                         ...
                         if (Test-Path claims_subfile_81_dec.sf)
                         {
                                                  ...
                                                  &$env:cmd\call_backup_mthly_sub81 sep 3
                        }
                }
                }
} else {
 if (Test-Path claims_subfile_81_oct.sf)
 {
                       if (Test-Path claims_subfile_81_nov.sf)
                       {
                         ...
                         if (Test-Path claims_subfile_81_dec.sf)
                         {
                                          ...
                                          &$env:cmd\call_backup_mthly_sub81 oct 4
                        }
                }
} else {
 if (Test-Path claims_subfile_81_nov.sf)
 {
                         if (Test-Path claims_subfile_81_dec.sf)
                         {
                                              ...
                                              &$env:cmd\call_backup_mthly_sub81 nov 5
                        }
} else {
 if (Test-Path claims_subfile_81_dec.sf)
 {
                                              &$env:cmd\call_backup_mthly_sub81 dec 0
} else {
 if (Test-Path claims_subfile_81_jan.sf)
 {
   if (Test-Path claims_subfile_81_feb.sf)
   {
        &$env:cmd\call_error_duplicate_files jan feb
  } else {
     if (Test-Path claims_subfile_81_mar.sf)
     {
            &$env:cmd\call_error_duplicate_files jan mar
    } else {
       if (Test-Path claims_subfile_81_apr.sf)
       {
                &$env:cmd\call_error_duplicate_files jan apr
      } else {
         if (Test-Path claims_subfile_81_may.sf)
         {
                       ...
                       if (Test-Path claims_subfile_81_nov.sf)
                       {
                         ...
                         if (Test-Path claims_subfile_81_dec.sf)
                         {
                                                    &$env:cmd\call_error_duplicate_files jan dec
                                                      ...
                                                      &$env:cmd\call_backup_mthly_sub81 jan 1
                        }
                }
                }
                }
                     ...
                     if (Test-Path claims_subfile_81_oct.sf)
                     {
                       ...
                       if (Test-Path claims_subfile_81_nov.sf)
                       {
                         ...
                         if (Test-Path claims_subfile_81_dec.sf)
                         {
                                              ...
                                              &$env:cmd\call_backup_mthly_sub81 feb 2
                        }
                }
                }
                }
                     ...
                     if (Test-Path claims_subfile_81_oct.sf)
                     {
                       ...
                       if (Test-Path claims_subfile_81_nov.sf)
                       {
                         ...
                         if (Test-Path claims_subfile_81_dec.sf)
                         {
                                          ...
                                          &$env:cmd\call_backup_mthly_sub81 mar 3
                        }
                }
                }
                }
                       ...
                       if (Test-Path claims_subfile_81_nov.sf)
                       {
                         ...
                         if (Test-Path claims_subfile_81_dec.sf)
                         {
                                                    &$env:cmd\call_error_duplicate_files apr dec
                                                      ...
                                                      &$env:cmd\call_backup_mthly_sub81 apr 4
                        }
                }
                }
                }
                     ...
                     if (Test-Path claims_subfile_81_oct.sf)
                     {
                       ...
                       if (Test-Path claims_subfile_81_nov.sf)
                       {
                         ...
                         if (Test-Path claims_subfile_81_dec.sf)
                         {
                                                  ...
                                                  &$env:cmd\call_backup_mthly_sub81 may 5
                        }
                }
                }
                }
