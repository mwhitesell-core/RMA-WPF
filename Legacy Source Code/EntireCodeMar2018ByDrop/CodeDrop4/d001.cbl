...
* 16/Sep/21 MC45 - undo change be2 which made OMA pricing = OHIP pricing as we now want separate prices based upon ohip or oma
* 17/Feb/01 MC46 - modify edit 12,17,29,49,50 and add new edit 69 to 81 for error 184 to 193
*                - modify  in  $use/d001_newu701_oma_code_edit.rtn & la5-oma-code-edit subroutine
*                - modify  in  $use/d001_newu701_oma_code_variables.ws, $use/d001_newu701_oma_code_variables_reset.rtn,
*		 - Yasemin requests to user to enter '!' when error  occurs fro errors 165, 186 & 187 to be the same as 163 & 164
*                - now check against const-effective-date instead of fee-effective-date for bill direct 
...
* MC46 - this also applies to edit 71 besides edit 17 and 49
        10  filler                              pic x(60)       value
...
* MC46 - this also applies to edit 75 as well
        10  filler                              pic x(60)       value
...
* MC46 - exclude G379 and include G297 and G250
...
* 2011/04/11 - end
* MC46 - include G244, G246, G240, G384, G379
...
* MC46 - include E423 from edit 29
*   if   hold-oma-cd(ss-clmdtl-oma) = 'S322' or 'S326' 
    if   hold-oma-cd(ss-clmdtl-oma) = 'S322' or 'S326' or 'E423'
* MC46 - end
* 2011/11/23 - end
*           (check the patient's age )
    then
...
*  MC46 - add edit 75 similar to edit 52

    if   hold-oma-cd(ss-clmdtl-oma) = 'K704'           
*           (check the patient's age )
    then
        move    hold-sv-date(ss-clmdtl-oma)     to ws-sv-date
        move    ws-pat-birth-date               to ws-birth-date
...
* MC46 - edit 74 
     ...
     if   (    hold-oma-cd(ss-clmdtl-oma) = 'G231'
            or hold-oma-cd(ss-clmdtl-oma) = 'G223'
            or hold-oma-cd(ss-clmdtl-oma) = 'Z228'
          )
      and  hold-sv-nbr-serv (ss-clmdtl-oma) > 1
      and  clmhdr-manual-review not = 'Y'
     then
...
* MC46 - edit 50 to exclude the check of G578 and G584 
...
* MC46 - if agent is bill direct, check service date against effective date in record 2 of f090 file;
*        otherwise, if agent is not bill direct, check service date against effective date in f040 file
    if     def-agent-bill-direct
    then
        if hold-sv-date (ss-clmdtl-oma) < const-effective-date (curr)
        then 
...
* MC46 - edit 70 
    ...
    if     hold-sv-day(ss-clmdtl-oma,1) = "BI" 
      and  hold-sv-nbr-serv(ss-clmdtl-oma) = 1
    then
	move 184			to err-ind
	perform za0-common-error	thru za0-99-exit
...
* 2012/04/16 - MC23
		...
		or 164
* 2012/04/16 - end
* MC46 - apply error 165, 186 & 187 to be the same as 163 & 164 as Yasemin requested
		or 165
		or 186 
		or 187
* MC46 - end  
...
* MC46
    if  (    site-id = "RMA"
	and (batctrl-agent-cd = 6)                  
	and (batctrl-bat-clinic-nbr-1-2 not = 98) 
	)
    then	
	move 194			to err-ind
	perform za0-common-error	thru za0-99-exit
