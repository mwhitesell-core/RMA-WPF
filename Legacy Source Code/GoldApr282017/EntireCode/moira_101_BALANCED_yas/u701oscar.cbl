...
* 13/Oct/07 MC33 - do not create suspend records if incoming agent=6 and doc-process-agent-6-flag <> 'Y'
*		 - request by Brad - if the the price we have in the f040 OMA Fee master is zero .. 
*		   in that case we need to use the incoming price and not the price on the f040 record . 
*		   modify in xx0-write-susp-diskout-dtl
*		 - 13/Nov/06 - Brad/Yas requested to set clmhdr-manual-review = 'Y' if using incoming price for
*		   f040 OMA Fee master is zero.
* 13/Oct/16 MC34 - include the check of oma code 'J'nnn and 'X'nnn with 'G'nnn for 0 referring doctor (see MC30).
...
* 14/Feb/25 MC36 - modify edit 12 to exclude check of Z316 (error 80)  $use/newu701_oscar_dtl_edit_check.rtn
* 14/Apr/23 MC37 - modify the criteria check for error 62 to exclude clinic 75 and location codes M526 & M527
*		   modify $use/newu701_oscar_error_messages.ws for error 62
* 14/May/21 MC38 - if incoming prov is '.' or '..', assume it is 'ON' set to clmhdr-health-care-prov.
*                - if incoming prov is invalid,     assume it is 'ON' set to clmhdr-health-care-prov.
* 14/Jun/05 MC39 - remove the edit check for err-ind = 94, modify$use/d001_newu701_oma_code_edit.rtn to comment out ws-hnnn
*                  & modify $use/newu701_oscar_dtl_edit_check.rtn to comment out err-ind = 94
* 14/Sep/08 MC40 - to add an edit check for terminated doctor (error 128)
*                - make changes in the copybook in $use/newu701_oscar_dtl_edit_check.rtn
*                  and add new error 128  in $use/newu701_oscar_error_messages.ws
* 14/Dec/04 MC41 - include province 'NU'
* 15/Jan/20 MC42 - add new error 129 for fatal error due to record misalign - it happened on HET (item record)
*                - check the rec type for 1st 3 bytes instead of the third byte
* 15/Mar/05 MC43 - add error "LOCATION Code not currently active for data entry" if loc-code has loc-active-for-entry = "N"
*                  in $use/newu701_oscar_error_messages.ws, the same check as in d001
* 15/Apr/09 MC44 - modify the edit 36 for error 65 in $use/newu701_oscar_dtl_edit_check.rtn
* 15/Jul/27 MC45 - modify the edit 12 for error 80 in $use/newu701_oscar_dtl_edit_check.rtn
* 15/Jul/30 MC46 - add display statements to debug the write to suspend-hdr with duplicate
* 16/May/03 MC47 - Yasemin requested to allow 231 days for  svc date billing before staled date
*                - make changes in the copybook in $use/newu701_oscar_dtl_edit_check.rtn
*                  and add new error in $use/newu701_oscar_error_messages.ws
* 17/Feb/09 MC48 - modify edit 12,17,29,49,50 and add new edit 69 to 81 for error 131 to 140
*                - add edits 69 to 81 in $use/newu701_oscar_dtl_edit_check.rtn,  add error message 131 to 140
*                - modify  in  $use/d001_newu701_oma_code_edit.rtn & la5-oma-code-edit subroutine
*                - modify  in  $use/d001_newu701_oma_code_variables.ws, $use/d001_newu701_oma_code_variables_reset.rtn,
*                  $use/newu701_oscar_error_messages.ws, $use/newu701_oscar_dtl_edit_check.rtn
* 17/Mar/09 MC49 - when determining current vs previous rates for OMA fee specifically for bill direct (agent 6)
...
* 17/Apr/26 MC50 - add edit 82 (error 141) - Process as bill direct, or change clinic to proper OHIP clinic  if clinic 98
*
*
...
* 2013/10/07 - MC33 - skip if incoming agent = 6 & doc-process-agent-6-flag <> 'Y'
   if unpriced-clmhdr-agent-cd = '6' and doc-process-agent-6-flag not =  'Y'
   then 
       move 'Y'				to skip-process-this-acct-id-flag
       go to fa0-99-exit.
*  endif
...
* MC38
*   if unpriced-clmhdr-hc-prov-cd = spaces 
    if unpriced-clmhdr-hc-prov-cd = spaces or '.' or '..'
* MC38 - end
    then 
        move 'ON'                           to      hdr-health-care-prov 
    else 
...
* MC49 - for bill direct , check with const-effective-date instead of fee-effective-date
...
* 2013/12/05 - MC35 - if none of oma code detail does not require referring doctor but it does exist, blank it out
...
tf0-write-addr-rec. 
	...
	write suspend-address-rec
        invalid key 
* MC46         move "D"            to skip-process-this-acct-id-flag
               go to tf0-99-exit. 
...
* 2013/12/05 - MC35 - check refer doc required or not
...
* 2013/12/05 - MC35 - initialize hold-oma-recs
        move spaces                     to hold-oma-recs
* 2013/12/05 -end
...
*  2013/10/09 - MC33
	...
	and hold-fee-incoming(ss-write-dtl) not = 0
    then
	move 'Y'			     to clmhdr-manual-review 
	move hold-fee-incoming(ss-write-dtl) to clmdtl-fee-ohip 
						clmdtl-fee-oma.
*   endif
*  2013/10/09 - end
