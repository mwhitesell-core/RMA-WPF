...
* 13/May/08 MC8  - add option 'B' to call m010_crm_d003.qks for bill direct notes
* 13/Jun/04 MC9  - add option 'I' to call m010_ins_d003.qks for insurance
* 		 - include f010_ins & f010_crm to check if record exists, so flag on the screen for user viewing
* 13/Jul/31 MC10 - comment out edit check for error 38 REFERRING Doctor can not be SAME as OHIP Doc
* 13/Oct/30 MC11 - show postal code x(10)
...
* 17/Apr/20 MC18 - reinstate the changes of bal due calculation:
*                - at the detail, display the clmdtl-fee-ohip for all agent;likewise,
*                  for calculating bal due, use ohip amt and paid amt for all agent
*                - now oma amt will not be the same as ohip amt for direct bill; thus display the oma amt
*		   at the detail and use oma amt for calculating bal due for direct bill
...
* 2011/02/09 - end
* 2013/05/08 - MC8
    88  flag-call-bd-notes-screen 		value "B".  
* 2013/05/08 - end
* 2013/06/04 - MC9
    88  flag-call-insurance-screen		value "I".  
* 2013/06/04 - end
...
* 2013/06/13 - MC9
77  status-cobol-f010-ins			pic x(2)        value zero.  
77  status-cobol-f010-crm 			pic x(2)        value zero.  
* 2013/06/13 - end
...
* MC18 
01  hold-ohip-oma-amt			pic s9(5)v99.
* MC18 - end
...
* 2013/06/13 - MC9
01  scr-display-literal.
    05  line 01 col 61 value "/".  
    05  line 01 col 64 value "/".  
    05  line 01 col 67 value "/".  
 
01  scr-display-flag. 
    05  line 01 col 59 pic x(2) using flag-pn.  
    05  line 01 col 62 pic x(2) using flag-cn.  
    05  line 01 col 65 pic x(2) using flag-pi.  
    05  line 01 col 68 pic x(2) using flag-ci.  
* 2013/06/13 - end
*  
01  scr-tech.  
...
*   05  line 06 col 17 pic x(6)  using ws-subscr-postal-cd.  
    05  line 06 col 17 pic x(10) using ws-subscr-postal-cd.  
* 2013/10/30 - end
* 2002/06/17 - MC
...
*   05  scr-acpt-postal-code line 06 col 17 pic x(6)  using ws-subscr-postal-cd.  
    05  scr-acpt-postal-code line 06 col 17 pic x(10) using ws-subscr-postal-cd.  
* 2013/10/30 - end
* 2002/06/17 - MC
...
*   05  line 23 col 12 pic zzzz9.99- using clmhdr-tot-claim-ar-ohip.  
    05  line 23 col 12 pic zzzz9.99- using hold-ohip-oma-amt.  
* MC18 - end
...
*   05  line 21 col 65 value "Upd  ~/C/D/E/G/H".  
    05  line 21 col 64 value "Upd ~/B/C/D/E/G/H".  
* 2013/05/08 - end
* 2011/02/09 - end
* 2007/01/30 - end
...
*   05  line 22 col 68 value       "M/O/P/R/T/*".  
    05  line 22 col 66 value     "I/M/O/P/R/T/*".  
* 2013/06/04 - end
* 2002/11/13 - end
...
* 2013/06/19 - MC9 - check if PN/CN/PI/CI records exist
    move spaces 			to flag-pn
					   flag-cn
					   flag-pi
					   flag-ci.

    perform xf6-access-f010-crm-ins		thru xf6-99-exit.

    display scr-display-literal.
    display scr-display-flag.
...
* 2013/05/08 - MC8
    if flag-call-bd-notes-screen
    then
	perform na2-call-bd-notes-pgm
					thru na2-99-exit.
*   endif
* 2013/05/08 - end

* 2013/06/04 - MC9
    if flag-call-insurance-screen
    then
	perform na3-call-insurance-pgm
					thru na3-99-exit.
*   endif
* 2013/06/04 - end
...
* 2011/02/09 - end
* 2013/05/08 - MC8
      and not flag-call-bd-notes-screen
* 2013/05/08 - end
* 2013/06/04 - MC9
      and not flag-call-insurance-screen
* 2013/06/04 - end
    then
	move spaces                           to   update-confirmation
	perform zb0-accept-confirm-update     thru zb0-99-exit
...
* 2013/07/31 - MC10 - comment out the below edit check 
...
* 2013/05/08 - MC8
na2-call-bd-notes-pgm.

*   (write claim nbr     to parameter file for passing to m010_crm_d003.qks)
    open output   parameter-file.
    move clmhdr-batch-nbr   		to parameter-batch-nbr.   
    move clmhdr-claim-nbr   		to parameter-claim-nbr.
    write parameter-rec.
    ...
    display load-message.
    move "quick term=vt220 auto=$pb_obj/m010_crm_d003.qkc debug=source" to macro.
    call  "SYSTEM" using macro.
...
* 2013/06/04 - MC9
na3-call-insurance-pgm.

*   (write claim nbr     to parameter file for passing to m010_ins_d003.qks)
    open output   parameter-file.
    move clmhdr-batch-nbr   		to parameter-batch-nbr.   
    move clmhdr-claim-nbr   		to parameter-claim-nbr.
    write parameter-rec.
    ...
    display load-message.
    move "quick term=vt220 auto=$pb_obj/m010_ins_d003.qkc debug=source" to macro.
    call  "SYSTEM" using macro.
...
*   move clmhdr-tot-claim-ar-ohip	to ws-tbl-orig-amt	(subs).  
    ...
    then
	 ...
	 move clmhdr-tot-claim-ar-oma   to ws-tbl-orig-amt	(subs).
*   endif  
* MC18 - end  
...
* 2011/02/09 - end
* 2013/05/08 - MC8
      or flag-call-bd-notes-screen
* 2013/05/08 - end
* 2013/06/04 - MC9
      or flag-call-insurance-screen
* 2013/06/04 - end
    then  
	next sentence  
    else  
...
* 2013/06/19 - MC9
xf6-access-f010-crm-ins.

* read f010-crm by patient ikey
    
    move spaces					to f010-crm-key.
    move clmhdr-pat-ohip-id-or-chart of claim-header-rec
						to key-pat-mstr of f010-crm-rec.

    start f010-crm key is greater than or equal to f010-crm-key 
	invalid key
	    go to xf6-20.          

    read f010-crm next into f010-crm-rec
	at end
	    go to xf6-20.       
   

    if key-pat-mstr of f010-crm-rec = clmhdr-pat-ohip-id-or-chart of claim-header-rec
    then
	if crm-clmhdr-batch-nbr = ' '
        then
	    move 'PN'				to flag-pn 
	else
            if     crm-clmhdr-batch-nbr = clmhdr-batch-nbr       
	      and  crm-clmhdr-claim-nbr = clmhdr-claim-nbr
            then
             	move 'CN'			to flag-cn.
*	    endif
*	endif
...
* read f010-crm by patient ikey + claim nbr

    move spaces					to f010-crm-key.
    move clmhdr-pat-ohip-id-or-chart of claim-header-rec
						to key-pat-mstr of f010-crm-rec.
    move clmhdr-batch-nbr			to crm-clmhdr-batch-nbr.
    move clmhdr-claim-nbr			to crm-clmhdr-claim-nbr.
    ...
    if     key-pat-mstr of f010-crm-rec = clmhdr-pat-ohip-id-or-chart     of claim-header-rec
      and  crm-clmhdr-batch-nbr = clmhdr-batch-nbr       
      and  crm-clmhdr-claim-nbr = clmhdr-claim-nbr
    then
       	move 'CN'			to flag-cn.
*   endif
...
* read f010-ins by patient ikey

    move spaces					to f010-ins-key.
    move clmhdr-pat-ohip-id-or-chart of claim-header-rec
						to key-pat-mstr of f010-ins-rec.

    start f010-ins key is greater than or equal to f010-ins-key 
	invalid key
	    go to xf6-40.          

    read f010-ins next into f010-ins-rec
	at end
	    go to xf6-40.       
  
    if key-pat-mstr of f010-ins-rec = clmhdr-pat-ohip-id-or-chart of claim-header-rec
    then
	if ins-clmhdr-batch-nbr = ' '
        then
	    move 'PI'				to flag-pi 
	else
            if     ins-clmhdr-batch-nbr = clmhdr-batch-nbr       
	      and  ins-clmhdr-claim-nbr = clmhdr-claim-nbr
            then
             	move 'CI'			to flag-ci.
*	    endif
*	endif
...
* read f010-ins by patient ikey + claim nbr

    move spaces					to f010-ins-key.
    move clmhdr-pat-ohip-id-or-chart of claim-header-rec
						to key-pat-mstr of f010-ins-rec.
    move clmhdr-batch-nbr			to ins-clmhdr-batch-nbr.
    move clmhdr-claim-nbr			to ins-clmhdr-claim-nbr.
    ...
    if     key-pat-mstr of f010-ins-rec = clmhdr-pat-ohip-id-or-chart     of claim-header-rec
      and  ins-clmhdr-batch-nbr = clmhdr-batch-nbr       
      and  ins-clmhdr-claim-nbr = clmhdr-claim-nbr
    then
       	move 'CI'			to flag-ci.
*   endif
...
* MC18 - reinstate the change below -  use clmdtl-fee-oma for direct bill; otherwise, use clmdtl-fee-ohip
...
*    then  
     if not direct-bill-agent  
     then  
	move clmhdr-tot-claim-ar-ohip	to hold-ohip-oma-amt
...
*	    clmhdr-manual-and-tape-paymnts  
	...
	move clmhdr-tot-claim-ar-oma 	to hold-ohip-oma-amt
 	add clmhdr-tot-claim-ar-oma  
 	    clmhdr-manual-and-tape-paymnts  
					giving hold-clmhdr-bal.  
* 2002/11/07 - end
