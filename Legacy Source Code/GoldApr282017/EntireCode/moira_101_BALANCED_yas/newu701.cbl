...
* 11/Nov/23 MC19 - modify edit 1, 5, 12, 13, 21, 29, 32, 34, 40, 45, 49, 50 plus add new edits 52 to 65
* 12/Apr/23 MC20 - reset field value when processing a new batch, change from speciality to specialty for spelling
* 12/Jul/04 MC21 - modify edit for error 69 to remove E022C check temporary
*		   in  $use/newu701_oscar_dtl_edit_check.rtn
* 12/Jul/12 MC22 - modify $use/pricing_logic.rtn to process yio before yf0 subroutine for pricing correctly
* 12/Oct/22 MC23 - since pricing for E676B is now correct, user requested to remove the error 123(edit 64) -'Check Fee for E676B'
*		   in  $use/newu701_oscar_dtl_edit_check.rtn & $use/d001_newu701_oma_code_edit.rtn
* 12/Nov/05 MC24 - allow to process agent 6, correct codes accordingly, only verify health care nbr if agent <> 6
* 13/Apr/11 MC25 - modify edit 1, 5, 12, 17 plus add new edits 66 to 69 for error 124 to 127
...
* 13/Jun/17 MC26 - set referring doctor nbr to be the same as billing doctor nbr if referring doctor nbr = 0 if
...
* 13/Jul/09 MC27 - Brad requested to add an edit check to make sure the service date > doctor start date
...
* 13/Jul/30      - UNDO MC27 -  to add an edit check to make sure the service date > doctor start date
*		 - make changes in the copybook in $use/newu701_oscar_dtl_edit_check.rtn
*		   and remove new error in $use/newu701_oscar_error_messages.ws
* 13/Oct/15 MC28 - include the check of oma code 'J'nnn and 'X'nnn with 'G'nnn for 0 referring doctor (see MC26).
*                  Linda O'Hara said no need to check on suffix as J & X codes allow suffix, A,B,C,M.
* 13/Dec/05 MC29 - if none of oma code detail requires referring doctor(ref-phy-ind in f040 = 'N') within the claim but
*                  referring doctor exists, then blank it out to reduce reject from MOH
* 14/Feb/25 MC30 - modify edit 12 to exclude check of Z316 (error 80)  $use/newu701_oscar_dtl_edit_check.rtn
* 14/Apr/01 MC31 - if incoming clinic nbr is wrong for the doctor, zero out clmhdr-doc-nbr
* 14/Apr/23 MC32 - modify the criteria check for error 62 to exclude clinic 75 and location codes M526 & M527
*                  modify $use/newu701_oscar_error_messages.ws for error 62
* 14/May/21 MC33 - if incoming prov is '.' or '..', assume it is 'ON' to clmhdr-health-care-prov.
*                - if incoming prov is invalid,     assume it is 'ON' to clmhdr-health-care-prov.
* 14/Jun/05 MC34 - remove the edit check for err-ind = 94, modify$use/d001_newu701_oma_code_edit.rtn to comment out ws-hnnn
*		   & modify $use/newu701_oscar_dtl_edit_check.rtn to comment out err-ind = 94
* 14/Sep/08 MC35 - to add an edit check for terminated doctor (error 128)                    
*		 - make changes in the copybook in $use/newu701_oscar_dtl_edit_check.rtn
*		   and add new error 128  in $use/newu701_oscar_error_messages.ws
* 14/Dec/04 MC36 - include province 'NU'
* 14/Dec/13 b.e. - Yas requested to change 120 detail record allowance to 135.
* (1387)           Found variable (ss-max-nbr-oma-det-rec-allow) set as 70 not 120 but set to 135 always
*	   	   And also updated  hold-oma-rec occurs to 135 in 'pricing_variables_hold.ws' 
*		   to keep it in sync (it was not 70 but 90???)
* 15/Jan/20 MC37 - change ss-max-nbr-oma-det-rec-allow to 90 from 70 because hold-oma-rec occurs 90 in 'pricing_variables_hold.ws'
*		 - add new error 129 for fatal error due to record misalign - it happened on HET (item record)
*		 - check the rec type for 1st 3 bytes instead of the third byte
* 15/Mar/05 MC38 - add error "LOCATION Code not currently active for data entry" if loc-code has loc-active-for-entry = "N"
*		   in $use/newu701_oscar_error_messages.ws, the same check as in d001
* 15/Apr/09 MC39 - modify the edit 36 for error 65 in $use/newu701_oscar_dtl_edit_check.rtn   
* 15/Jul/27 MC40 - modify the edit 12 for error 130 in $use/newu701_oscar_dtl_edit_check.rtn   
* 15/Jul/30 MC41 - add display statements to debug the write to suspend-hdr with duplicate      
* 16/May/03 MC42 - Yasemin requested to allow 231 days for  svc date billing before staled date
*		 - make changes in the copybook in $use/newu701_oscar_dtl_edit_check.rtn
*		   and add new error in $use/newu701_oscar_error_messages.ws
* 17/Feb/09 MC43 - modify edit 12,17,29,49,50 and add new edit 69 to 81 for error 131 to 140
*                - add edits 69 to 81 in $use/newu701_oscar_dtl_edit_check.rtn,  add error message 131 to 140 
*                - modify  in  $use/d001_newu701_oma_code_edit.rtn & la5-oma-code-edit subroutine
*                - modify  in  $use/d001_newu701_oma_code_variables.ws, $use/d001_newu701_oma_code_variables_reset.rtn,      
*		   $use/newu701_oscar_error_messages.ws, $use/newu701_oscar_dtl_edit_check.rtn
*                  and add new error 131 to 140 in $use/newu701_oscar_error_messages.ws
* 17/Mar/09 MC44 - when determining current vs previous rates for OMA fee specifically for bill direct (agent 6)
*		   use the effective date in record 2 of constants master       
*                  undo change be2 which made OMA pricing = OHIP pricing as we now want separate prices based upon ohip or oma
* 17/Apr/26 MC45 - add edit 82 (error 141) - Process as bill direct, or change clinic to proper OHIP clinic  if clinic 98
...
* MC44 - for bill direct , check with const-effective-date instead of fee-effective-date
...
* 2013/12/05 - MC29 - if none of oma code detail does not require referring doctor but it does exist, blank it out
