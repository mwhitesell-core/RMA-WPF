...
// 2005/jan/21 b.e. - add `con`tact designer routine to maintain doctor`s contacts
// 2005/apr/26 M.C. - add `more` a  designer routine to maintain doctor`s extra info
// 2005/feb/02 b.e. - changed `more` call to m029 to m020b
// - added call to `foll`owups subscreen (m029)
// 2005/mar/30 M.C. - pass doc-dept to call m020a
// - include clinic nbr status & specialty effective date
// - remove doctor home and office address lines because they
// have transferred to f028-contacts-info-mstr
// - combine clinic/batch nbr/status together
// - combine specialty and effective date together
// - add f020-doctor-extra
// 2006/02/28 M.C.  - added call to `div`ision subscreen (m033) for HSC
// 2006/04/04 M.C.  - suppress changing of doc-name, name should be changed
// from m027 screen only 
// 2006/06/06 M.C.  - pass f020 record when calling m027 screen
// 2006/09/12 M.C.  - when user enters termination date, prompt replacement doc nbr
// and when user zero out termination date, blank out replacement doc nbr
// 2006/oct/02 M.C. - define f020 as reference file
// 2006/oct/17 M.C. - when user enters termination date and with replacement doc nbr, run
// a command `transfer_f020_f027_f028` to transfer from doctor records to
// replaced by doc nbr
// 2006/oct/18 M.C. - Brad suggested to blank specialty cd when zero
// 2006/oct/19 M.C. - add prior-doc-nbr on the screen next to doc nbr
// - when user terminates doctor with replacement doc nbr, also update
// `prior-doc-nbr` in the replacement doctor record equal to terminated doctor
// 2006/nov/09 M.C. - Yas said that 00 specialty cd is a a valid code
// 2008/oct/26 b.e. - upshift doctor `nbr` field so letters are uppercase only 
// 2011/aug/17 MC1  - if entry mode, make sure doc nbr does not exist in f021 file
// - undo MC1 on 2014/Feb/10 because Yasemin does not want to add new doctors via
// doctor assignment subsystem, it is not feasible based on how Jane assigns the new doctors
// 2013/apr/03 MC2  - add designer `rpt` to define any rpt generation exception
// 2013/Sep/23 MC3  - modify to check clinic nbr edit properly by each individual clinic
// - if doc dept = 31 (solo doctors), allow clinic 51 to 59 if environment is 101c;
// because clinic 51 to 59 will not be defined in 101c;
// otherwise, the clinic must be defined in its own environment (mp or solo)
// 2013/oct/02 MC4  - for some reasons that loc 6/12/18/24/30 were disable for entry because it was defined as display
// these entries have been skipped in entry mode but they are okay in find mode, remove display on fields
// 2013/Nov/26 MC5  - do not allow blank doctor name and doctor initials
// 2014/jun/10 MC6  - add f020-doctor-audit to capture before change records
...
// 2014/Oct/29 MC7  - add opt when get iconst-mstr-rec
// - change from postfind to process of doc-dept to determine the app-version for all modes
// - applied the same check in preupdate as well
// 2015/Feb/09 MC8  - add back to determine the app-version in postfind procedure  in order to assign the clinic nbr
// 2015/Apr/08 MC9  - Brad requested to check for oscar-provider as well when changing doc nbr (replaced-by-doc).  
// Provide warning if record exists so that user can inform Yasemin to confirm
// 2016/Apr/14 MC10 - redisplay doc-name and doc-inits after returning from `cont`
// - do not allow to change clinic nbr if there is batch nbr
...
namespace rma.Views
{
    ...
    partial class Billing_M020 : BasePage
    {
        ...
        private void Page_Load(object sender, RoutedEventArgs e)
        {
            ...
            dsrDesigner_CONT.Click += dsrDesigner_CONT_Click;
            dsrDesigner_MORE.Click += dsrDesigner_MORE_Click;
            dsrDesigner_FOLL.Click += dsrDesigner_FOLL_Click;
            dsrDesigner_RPT.Click += dsrDesigner_RPT_Click;
            ...
            dsrDesigner_LIN7.Click += dsrDesigner_LIN7_Click;
            ...
            fldF020_DOCTOR_MSTR_DOC_INITS.Edit += fldF020_DOCTOR_MSTR_DOC_INITS_Edit;
            fldF020_DOCTOR_MSTR_DOC_NAME.Edit += fldF020_DOCTOR_MSTR_DOC_NAME_Edit;
            ...
            fldF020_DOCTOR_MSTR_DOC_DEPT.Process += fldF020_DOCTOR_MSTR_DOC_DEPT_Process;
        ...
        protected override void SetVariables()
        {
            ...
            X_DOC_REPLACED_FLAG = new CoreCharacter("X_DOC_REPLACED_FLAG", 1, this, Common.cEmptyString);
            X_DOC_OSCAR_REPLACED = new CoreCharacter("X_DOC_OSCAR_REPLACED", 1, this, Common.cEmptyString);
            ...
            fleF020_REF = new SqlFileObject(this, FileTypes.Reference, 0, "INDEXED", "F020_DOCTOR_MSTR", "F020_REF", false, false, false, 0, "m_cnnQUERY");
            fleF020_EXTRA_DES = new SqlFileObject(this, FileTypes.Designer, 0, "INDEXED", "F020_DOCTOR_EXTRA", "F020_EXTRA_DES", false, false, false, 0, "m_trnTRANS_UPDATE");
            ...
            fleTRANSFER_FILE = new SqlFileObject(this, FileTypes.Designer, 0, "SEQUENTIAL", "TRANSFER_FILE", "", false, false, false, 0, "m_trnTRANS_UPDATE", FileType.SequentialDataBase);
            ...
            fleF200_OSCAR_PROVIDER = new SqlFileObject(this, FileTypes.Designer, 0, "INDEXED", "F200_OSCAR_PROVIDER", "", false, false, false, 0, "m_trnTRANS_UPDATE");
            ...
            fleF020_REF.Access += fleF020_REF_Access;
            fleF020_EXTRA_DES.Access += fleF020_EXTRA_DES_Access;
        ...
        private CoreCharacter X_DOC_REPLACED_FLAG;
        private CoreCharacter X_DOC_OSCAR_REPLACED;
        ...
        private void FleF020_DOCTOR_MSTR_SetItemFinals()
        {
            ...
            if (T_DOC_DATE_FAC_TERM.Value != 0)
            {
        ...
        private void fleF020_DOCTOR_EXTRA_Access(ref string AccessClause)
        {
            ...
            try
            {
                ...
                strText.Append(" WHERE ").Append(fleF020_DOCTOR_EXTRA.ElementOwner("DOC_NBR")).Append(" = ").Append(Common.StringToField(fleF020_DOCTOR_MSTR.GetStringValue("DOC_NBR")));
        ...
        private SqlFileObject fleF020_REF;

        private void fleF020_REF_Access(ref string AccessClause)
        {
            ...
            try
            {
                ...
                strText.Append(" WHERE ").Append(fleF020_REF.ElementOwner("DOC_NBR")).Append(" = ").Append(Common.StringToField(fleF020_DOCTOR_EXTRA.GetStringValue("REPLACED_BY_DOC_NBR")));

                strText.Append(" ORDER BY ").Append(fleF020_REF.ElementOwner("DOC_NBR"));
                AccessClause = strText.ToString();
        ...
        private SqlFileObject fleF020_EXTRA_DES;

        private void fleF020_EXTRA_DES_Access(ref string AccessClause)
        {
            ...
            try
            {
                ...
                strText.Append(" WHERE ").Append(fleF020_EXTRA_DES.ElementOwner("DOC_NBR")).Append(" = ").Append(Common.StringToField(fleF020_DOCTOR_EXTRA.GetStringValue("REPLACED_BY_DOC_NBR")));

                strText.Append(" ORDER BY ").Append(fleF020_EXTRA_DES.ElementOwner("DOC_NBR"));
                AccessClause = strText.ToString();
        ...
        //# NOTE: This region is placeholder and is required by the RenaissanceArchitectPreCompilerAddIn.
        //# Do not delete, modify or move it.  Updated: 5/29/2017 10:10:36 AM
        ...
        private void Initialize_TRANS_UPDATE()
        {
            ...
            fleF020_EXTRA_DES.Transaction = m_trnTRANS_UPDATE;
        ...
        protected override void InitializeFiles()
        {
            ...
            {
                Initialize_TRANS_UPDATE();
                fleF020_REF.Connection = m_cnnQUERY;
        ...
        //# Precompiler Ver.: 1.0.6353.18277  Generated on: 5/29/2017 10:10:37 AM
        //#-----------------------------------------
        ...
        public override void BindFields()
        {
            try
            {
                ...
                //fldF020_DOCTOR_EXTRA_DOC_CLINIC_NBR_STATUS.Bind(fleF020_DOCTOR_EXTRA);
                ...
                //fldF020_DOCTOR_EXTRA_DOC_CLINIC_NBR_2_STATUS.Bind(fleF020_DOCTOR_EXTRA);
                ...
                //fldF020_DOCTOR_EXTRA_DOC_CLINIC_NBR_3_STATUS.Bind(fleF020_DOCTOR_EXTRA);
                ...
                //fldF020_DOCTOR_EXTRA_DOC_CLINIC_NBR_4_STATUS.Bind(fleF020_DOCTOR_EXTRA);
                ...
                //fldF020_DOCTOR_EXTRA_DOC_CLINIC_NBR_5_STATUS.Bind(fleF020_DOCTOR_EXTRA);
                ...
                //fldF020_DOCTOR_EXTRA_DOC_CLINIC_NBR_6_STATUS.Bind(fleF020_DOCTOR_EXTRA);
                fldF020_DOCTOR_MSTR_DOC_SPEC_CD.Bind(fleF020_DOCTOR_MSTR);
                fldF020_DOCTOR_EXTRA_DOC_SPEC_CD_EFF_DATE.Bind(fleF020_DOCTOR_EXTRA);
                fldF020_DOCTOR_MSTR_DOC_SPEC_CD_2.Bind(fleF020_DOCTOR_MSTR);
                fldF020_DOCTOR_EXTRA_DOC_SPEC_CD_2_EFF_DATE.Bind(fleF020_DOCTOR_EXTRA);
                fldF020_DOCTOR_MSTR_DOC_SPEC_CD_3.Bind(fleF020_DOCTOR_MSTR);
                fldF020_DOCTOR_EXTRA_DOC_SPEC_CD_3_EFF_DATE.Bind(fleF020_DOCTOR_EXTRA);
        ...
        private void fldF020_DOCTOR_MSTR_DOC_INITS_Edit()
        {
            ...
            try
            {
                ...
                {
                    ErrorMessage(QDesign.NULL("Doctor initials cannot be BLANK"));
        ...
        private void fldF020_DOCTOR_MSTR_DOC_NAME_Edit()
        {
            ...
            try
            {
                ...
                if (QDesign.NULL(FieldText) == QDesign.NULL(" "))
                {
                    ErrorMessage(QDesign.NULL("Doctor name cannot be BLANK"));
                    // TODO: May need to fix manually
                }
        ...
        private bool Internal_CHECK_CLINIC()
        {
            ...
            try
            {
                ...
                {
                    if (QDesign.NULL(fleF020_DOCTOR_MSTR.GetDecimalValue("DOC_DEPT")) == 31 & (QDesign.NULL(APP_VERSION.Value) == QDesign.NULL(VERSION_LIVE.Value) | QDesign.NULL(APP_VERSION.Value) == QDesign.NULL("101")))
                    {
                        if (QDesign.NULL(X_CLINIC_NBR.Value) > 50 & QDesign.NULL(X_CLINIC_NBR.Value) < 60)
                        {
                            Warning("Clinic 50 is okay");
                        }
                        else
                        {
                            ErrorMessage("WRONG CLINIC IS NOT ALLOWED, PLEASE RE-ENTER");
                        }
                    }
                    else
                    {
        ...
        private void fldF020_DOCTOR_MSTR_DOC_CLINIC_NBR_Edit()
        {
            ...
            try
            {
                ...
                if (ChangeMode & QDesign.NULL(fleF020C_DOC_CLINIC_NEXT_BATCH_NBR.GetDecimalValue("DOC_NX_AVAIL_BATCH")) != 0)
                {
                    ErrorMessage(" You cannot change clinic because there is batch nbr. \a");
                }
        ...
        private void fldF020_DOCTOR_MSTR_DOC_CLINIC_NBR_2_Edit()
        {

            try
            {

                X_CLINIC_NBR.Value = fleF020_DOCTOR_MSTR.GetDecimalValue("DOC_CLINIC_NBR_2");
                Internal_CHECK_CLINIC();
                if (ChangeMode & QDesign.NULL(fleF020_DOCTOR_MSTR.GetDecimalValue("DOC_NX_AVAIL_BATCH_2")) != 0)
                {
                    ErrorMessage(" You cannot change clinic because there is batch nbr. \a");
                }
        ...
        private void fldF020_DOCTOR_MSTR_DOC_CLINIC_NBR_3_Edit()
        {

            try
            {

                X_CLINIC_NBR.Value = fleF020_DOCTOR_MSTR.GetDecimalValue("DOC_CLINIC_NBR_3");
                Internal_CHECK_CLINIC();
                if (ChangeMode & QDesign.NULL(fleF020_DOCTOR_MSTR.GetDecimalValue("DOC_NX_AVAIL_BATCH_3")) != 0)
                {
                    ErrorMessage(" You cannot change clinic because there is batch nbr. \a");
                }
        ...
        private void fldF020_DOCTOR_MSTR_DOC_CLINIC_NBR_4_Edit()
        {

            try
            {

                X_CLINIC_NBR.Value = fleF020_DOCTOR_MSTR.GetDecimalValue("DOC_CLINIC_NBR_4");
                Internal_CHECK_CLINIC();
                if (ChangeMode & QDesign.NULL(fleF020_DOCTOR_MSTR.GetDecimalValue("DOC_NX_AVAIL_BATCH_4")) != 0)
                {
                    ErrorMessage(" You cannot change clinic because there is batch nbr. \a");
                }
        ...
        private void fldF020_DOCTOR_MSTR_DOC_CLINIC_NBR_5_Edit()
        {

            try
            {

                X_CLINIC_NBR.Value = fleF020_DOCTOR_MSTR.GetDecimalValue("DOC_CLINIC_NBR_5");
                Internal_CHECK_CLINIC();
                if (ChangeMode & QDesign.NULL(fleF020_DOCTOR_MSTR.GetDecimalValue("DOC_NX_AVAIL_BATCH_5")) != 0)
                {
                    ErrorMessage(" You cannot change clinic because there is batch nbr. \a");
                }
        ...
        private void fldF020_DOCTOR_MSTR_DOC_CLINIC_NBR_6_Edit()
        {

            try
            {

                X_CLINIC_NBR.Value = fleF020_DOCTOR_MSTR.GetDecimalValue("DOC_CLINIC_NBR_6");
                Internal_CHECK_CLINIC();
                if (ChangeMode & QDesign.NULL(fleF020_DOCTOR_MSTR.GetDecimalValue("DOC_NX_AVAIL_BATCH_6")) != 0)
                {
                    ErrorMessage(" You cannot change clinic because there is batch nbr. \a");
                }
        ...
        private void dsrDesigner_CONT_Click(object sender, System.EventArgs e)
        {
            ...
            try
            {
                ...
                X_DOC_NBR.Value = fleF020_DOCTOR_MSTR.GetStringValue("DOC_NBR");
                X_DOC_NAME.Value = QDesign.RTrim(QDesign.LeftJustify(fleF020_DOCTOR_MSTR.GetStringValue("DOC_NAME"))) + "," + fleF020_DOCTOR_MSTR.GetStringValue("DOC_INIT1") + fleF020_DOCTOR_MSTR.GetStringValue("DOC_INIT2") + fleF020_DOCTOR_MSTR.GetStringValue("DOC_INIT3");
                //Parent:DOC_INITS
                object[] arrRunscreen = { fleF020_DOCTOR_MSTR, X_DOC_NBR, X_DOC_NAME };
                RunScreen(new Billing_M027(), RunScreenModes.Same, ref arrRunscreen);
                Display(ref fldF020_DOCTOR_MSTR_DOC_NAME);
                Display(ref fldF020_DOCTOR_MSTR_DOC_INITS);
        ...
        private void dsrDesigner_MORE_Click(object sender, System.EventArgs e)
        {
            ...
            try
            {
                ...
                //Parent:DOC_INITS
                X_DOC_DEPT.Value = fleF020_DOCTOR_MSTR.GetDecimalValue("DOC_DEPT");
                object[] arrRunscreen = { X_DOC_NBR, X_DOC_NAME, X_DOC_DEPT };
                RunScreen(new Billing_M020A(), RunScreenModes.Same, ref arrRunscreen);
        ...
        private void dsrDesigner_FOLL_Click(object sender, System.EventArgs e)
        {
            ...
            try
            {
                ...
                X_DOC_NAME.Value = QDesign.RTrim(QDesign.LeftJustify(fleF020_DOCTOR_MSTR.GetStringValue("DOC_NAME"))) + "," + fleF020_DOCTOR_MSTR.GetStringValue("DOC_INIT1") + fleF020_DOCTOR_MSTR.GetStringValue("DOC_INIT2") + fleF020_DOCTOR_MSTR.GetStringValue("DOC_INIT3");
                //Parent:DOC_INITS
                object[] arrRunscreen = { X_DOC_NBR, X_DOC_NAME };
                RunScreen(new Billing_M029(), RunScreenModes.Same, ref arrRunscreen);
        ...
        private void dsrDesigner_RPT_Click(object sender, System.EventArgs e)
        {
            ...
            try
            {
                ...
                RunScreen(new Billing_M020B(), RunScreenModes.Same, ref arrRunscreen);
        ...
        private void dsrDesigner_LIN6_Click(object sender, System.EventArgs e)
        {
            ...
            try
            {
                ...
                //Accept(ref fldF020_DOCTOR_EXTRA_DOC_CLINIC_NBR_STATUS);
                //Accept(ref fldF020_DOCTOR_MSTR_DOC_CLINIC_NBR_2);
                //Accept(ref fldF020_DOCTOR_EXTRA_DOC_CLINIC_NBR_2_STATUS);
                //Accept(ref fldF020_DOCTOR_MSTR_DOC_CLINIC_NBR_3);
                //Accept(ref fldF020_DOCTOR_EXTRA_DOC_CLINIC_NBR_3_STATUS);
                //Accept(ref fldF020_DOCTOR_MSTR_DOC_CLINIC_NBR_4);
                //Accept(ref fldF020_DOCTOR_EXTRA_DOC_CLINIC_NBR_4_STATUS);
                //Accept(ref fldF020_DOCTOR_MSTR_DOC_CLINIC_NBR_5);
                //Accept(ref fldF020_DOCTOR_EXTRA_DOC_CLINIC_NBR_5_STATUS);
                //Accept(ref fldF020_DOCTOR_MSTR_DOC_CLINIC_NBR_6);
                //Accept(ref fldF020_DOCTOR_EXTRA_DOC_CLINIC_NBR_6_STATUS);
        ...
        private void dsrDesigner_LIN7_Click(object sender, System.EventArgs e)
        {
            ...
            try
            {
                ...
                Accept(ref fldF020_DOCTOR_EXTRA_DOC_SPEC_CD_EFF_DATE);
                Accept(ref fldF020_DOCTOR_MSTR_DOC_SPEC_CD_2);
                Accept(ref fldF020_DOCTOR_EXTRA_DOC_SPEC_CD_2_EFF_DATE);
                Accept(ref fldF020_DOCTOR_MSTR_DOC_SPEC_CD_3);
                Accept(ref fldF020_DOCTOR_EXTRA_DOC_SPEC_CD_3_EFF_DATE);
        ...
        private void dsrDesigner_LIN5_Click(object sender, System.EventArgs e)
        {
            ...
            try
            {
                ...
                if (QDesign.NULL(T_DOC_DATE_FAC_TERM.Value) == 0)
                {
                    ...
                    fleF020_DOCTOR_EXTRA.set_SetValue("REPLACED_BY_DOC_NBR", " ");
                    Display(ref fldF020_DOCTOR_EXTRA_REPLACED_BY_DOC_NBR);
                }
                else
                {
                ...
                if (QDesign.NULL(T_DOC_DATE_FAC_TERM.Value) != 0)
                {
                    ...
                    Accept(ref fldF020_DOCTOR_EXTRA_REPLACED_BY_DOC_NBR);
                    if (QDesign.NULL(fleF020_DOCTOR_EXTRA.GetStringValue("REPLACED_BY_DOC_NBR")) == QDesign.NULL(fleF020_DOCTOR_MSTR.GetStringValue("DOC_NBR")))
                    {
                        ErrorMessage(QDesign.NULL("Replaced by Doc cannot be the same as the current doctor nbr"));
                        // TODO: May need to fix manually
                    }
                    if (QDesign.NULL(fleF020_DOCTOR_EXTRA.GetStringValue("REPLACED_BY_DOC_NBR")) != QDesign.NULL(" "))
                    {
                        // --> GET F020_REF <--
                        m_strWhere = new StringBuilder(" WHERE ");
                        m_strWhere.Append(" ").Append(fleF020_REF.ElementOwner("DOC_NBR")).Append(" = ");
                        m_strWhere.Append(Common.StringToField(fleF020_DOCTOR_EXTRA.GetStringValue("REPLACED_BY_DOC_NBR")));

                        fleF020_REF.GetData(m_strWhere.ToString(), GetDataOptions.IsOptional);
                        // --> End GET F020_REF <--
                        if (!AccessOk)
                        {
                            ErrorMessage(QDesign.NULL("Doctor nbr not defined"));
                            // TODO: May need to fix manually
                        }
                        else
                        {
                            if (QDesign.NULL(fleF020_REF.GetDecimalValue("DOC_OHIP_NBR")) != QDesign.NULL(fleF020_DOCTOR_MSTR.GetDecimalValue("DOC_OHIP_NBR")))
                            {
                                ErrorMessage("Replaced Doctor does not have the same doc ohip nbr");
                            }
                            if (QDesign.NULL(fleF020_REF.GetNumericDateValue("DOC_DATE_FAC_TERM")) != 0)
                            {
                                ErrorMessage("Replaced Doctor has been terminated");
                            }
                            X_DOC_REPLACED_FLAG.Value = "Y";
                        }
                        // --> GET F200_OSCAR_PROVIDER <--
                        m_strWhere = new StringBuilder(" WHERE ");
                        m_strWhere.Append(" ").Append(fleF200_OSCAR_PROVIDER.ElementOwner("OSCAR_PROVIDER_NO")).Append(" = ");
                        m_strWhere.Append(Common.StringToField(fleF020_DOCTOR_MSTR.GetStringValue("DOC_NBR")));

                        fleF200_OSCAR_PROVIDER.GetData(m_strWhere.ToString(), GetDataOptions.IsOptional);
                        // --> End GET F200_OSCAR_PROVIDER <--
                        if (AccessOk)
                        {
                            Warning(QDesign.NULL("Doctor nbr will change in f200-oscar-provider table, please inform Yasemin about it."));
                            // TODO: May need to fix manually
                            X_DOC_OSCAR_REPLACED.Value = "Y";
                            fleF200_OSCAR_PROVIDER.set_SetValue("DOC_NBR", fleF020_DOCTOR_EXTRA.GetStringValue("REPLACED_BY_DOC_NBR"));
                        }
                    }
                }
        ...
        private void dsrDesigner_EXP_Click(object sender, System.EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    X_DOC_ADDR_HOME_PC.Value = fleF020_DOCTOR_MSTR.GetStringValue("DOC_ADDR_HOME_PC1") + QDesign.ASCII(fleF020_DOCTOR_MSTR.GetDecimalValue("DOC_ADDR_HOME_PC2"), 1) + fleF020_DOCTOR_MSTR.GetStringValue("DOC_ADDR_HOME_PC3") + QDesign.ASCII(fleF020_DOCTOR_MSTR.GetDecimalValue("DOC_ADDR_HOME_PC4"), 1) + fleF020_DOCTOR_MSTR.GetStringValue("DOC_ADDR_HOME_PC5") + QDesign.ASCII(fleF020_DOCTOR_MSTR.GetDecimalValue("DOC_ADDR_HOME_PC6"), 1);
                    //Parent:DOC_ADDR_HOME_PC
                }
                ...
                {
                    X_DOC_ADDR_OFFICE_PC.Value = fleF020_DOCTOR_MSTR.GetStringValue("DOC_ADDR_OFFICE_PC1") + QDesign.ASCII(fleF020_DOCTOR_MSTR.GetDecimalValue("DOC_ADDR_OFFICE_PC2"), 1) + fleF020_DOCTOR_MSTR.GetStringValue("DOC_ADDR_OFFICE_PC3") + QDesign.ASCII(fleF020_DOCTOR_MSTR.GetDecimalValue("DOC_ADDR_OFFICE_PC4"), 1) + fleF020_DOCTOR_MSTR.GetStringValue("DOC_ADDR_OFFICE_PC5") + QDesign.ASCII(fleF020_DOCTOR_MSTR.GetDecimalValue("DOC_ADDR_OFFICE_PC6"), 1);
                    //Parent:DOC_ADDR_OFFICE_PC
                }
                ...
                //COMLINE.Value = "$cmd/batch_copy_bank_info_out " + QDesign.RTrim(QDesign.LeftJustify(UserID)) + " " + "f020" + " " + fleF020_DOCTOR_MSTR.GetStringValue("DOC_NBR") + " " + "'" + QDesign.RTrim(QDesign.LeftJustify(X_DOC_NAME.Value)) + "'" + " " + "'" + QDesign.RTrim(QDesign.LeftJustify(X_DOC_INITS.Value)) + "'" + " " + QDesign.ASCII(fleF020_DOCTOR_MSTR.GetDecimalValue("DOC_OHIP_NBR"), 6) + " " + QDesign.ASCII(fleF020_DOCTOR_MSTR.GetDecimalValue("DOC_SIN_NBR"), 9) + " " + "'" + QDesign.RTrim(QDesign.LeftJustify(X_DOC_SUB_SPECIALTY.Value)) + "'" + " " + QDesign.ASCII(fleF020_DOCTOR_MSTR.GetDecimalValue("DOC_DEPT"), 2) + " " + QDesign.ASCII(fleF020_DOCTOR_MSTR.GetDecimalValue("DOC_CLINIC_NBR"), 2) + " " + "'" + QDesign.RTrim(QDesign.LeftJustify(X_DOC_FULL_PART_IND.Value)) + "'" + " " + "'" + QDesign.RTrim(QDesign.LeftJustify(X_DOC_ADDR_HOME_1.Value)) + "'" + " " + "'" + QDesign.RTrim(QDesign.LeftJustify(X_DOC_ADDR_HOME_2.Value)) + "'" + " " + "'" + QDesign.RTrim(QDesign.LeftJustify(X_DOC_ADDR_HOME_3.Value)) + "'" + " " + "'" + QDesign.RTrim(QDesign.LeftJustify(X_DOC_ADDR_HOME_PC.Value)) + "'" + " " + "'" + QDesign.RTrim(QDesign.LeftJustify(X_DOC_ADDR_OFFICE_1.Value)) + "'" + " " + "'" + QDesign.RTrim(QDesign.LeftJustify(X_DOC_ADDR_OFFICE_2.Value)) + "'" + " " + "'" + QDesign.RTrim(QDesign.LeftJustify(X_DOC_ADDR_OFFICE_3.Value)) + "'" + " " + "'" + QDesign.RTrim(QDesign.LeftJustify(X_DOC_ADDR_OFFICE_PC.Value)) + "'" + " " + QDesign.ASCII(fleF020_DOCTOR_MSTR.GetDecimalValue("DOC_BANK_NBR"), 4) + " " + QDesign.ASCII(fleF020_DOCTOR_MSTR.GetDecimalValue("DOC_BANK_BRANCH"), 5) + " " + "'" + QDesign.RTrim(QDesign.LeftJustify(X_DOC_BANK_ACCT.Value)) + "'" + " " + "'" + QDesign.ASCII(fleF020_DOCTOR_MSTR.GetNumericDateValue("DOC_DATE_FAC_START"), 8) + "'" + " " + "'" + QDesign.ASCII(T_DOC_DATE_FAC_TERM.Value, 8) + "'";
                ...
                if (!Directory.Exists(ConfigurationManager.AppSettings["FlatFilePath"].Replace("UserID", Environment.UserName) + "\\temp\\transfer_area\\rmabill"))
                {
                    Directory.CreateDirectory(ConfigurationManager.AppSettings["FlatFilePath"].Replace("UserID", Environment.UserName) + "\\temp\\transfer_area\\rmabill");
                }

                var sw = new StreamWriter(ConfigurationManager.AppSettings["FlatFilePath"].Replace("UserID", Environment.UserName) + "\\temp\\transfer_area\\rmabill\\transfer.dat");
                foreach (var o in arrPS)
                {
                ...
                sw = new StreamWriter(ConfigurationManager.AppSettings["FlatFilePath"].Replace("UserID", Environment.UserName) + "\\temp\\transfer_area\\rmabill\\brad.dat");
                foreach (var o in arrPS)
                {
                    sw.WriteLine(o.ToString());
                }
        ...
        private bool Internal_SAVEF020AUDIT()
        {
            ...
            try
            {
                ...
                //A_DOC_EP_PED.Value = fleF020_DOCTOR_MSTR.GetNumericDateValue("DOC_EP_PED");
        ...
        protected override bool PreUpdate()
        {
            ...
            try
            {
                ...
                //if (QDesign.NULL(fleF020_DOCTOR_MSTR.GetDecimalValue("DOC_CLINIC_NBR")) > 50 & QDesign.NULL(fleF020_DOCTOR_MSTR.GetDecimalValue("DOC_CLINIC_NBR")) < 60 & QDesign.NULL(fleF020_DOCTOR_MSTR.GetDecimalValue("DOC_DEPT")) != 31)
                //{
                //    ErrorMessage("Clinic 50`s is not allowed for dept <> 31");
                //}
                if (fleF020_DOCTOR_MSTR.NewRecord | fleF020_DOCTOR_EXTRA.NewRecord)
                {
                ...
                if (QDesign.NULL(X_DOC_DEPT_FLAG.Value) == QDesign.NULL("Y"))
                {
                    ...
                    else
                    {
                        ...
                        object[] arrPS = { fleF020_DOCTOR_MSTR.GetStringValue("DOC_NBR"),
                            QDesign.ASCII(fleF020_DOCTOR_MSTR.GetDecimalValue("DOC_DEPT"), 2),
                            QDesign.ASCII(W_START_DATE.Value, 8) };
                        string ps = System.Configuration.ConfigurationManager.AppSettings["JobScriptPath"] + @"batch_update_f050_f051_f060.ps1";
                        string psServer = System.Configuration.ConfigurationManager.AppSettings["psServer"].ToString();
                        PowerShellEngine pse = new PowerShellEngine();
                        pse.ExecuteScriptFile(ps, arrPS, psServer);
                        pse.Dispose();
                ...
                if (QDesign.NULL(X_DOC_REPLACED_FLAG.Value) == QDesign.NULL("Y"))
                {
                    // --> GET F020_EXTRA_DES <--

                    fleF020_EXTRA_DES.GetData(GetDataOptions.IsOptional);
                    // --> End GET F020_EXTRA_DES <--
                    if (AccessOk)
                    {
                        fleF020_EXTRA_DES.set_SetValue("PRIOR_DOC_NBR", fleF020_DOCTOR_MSTR.GetStringValue("DOC_NBR"));
                        fleF020_EXTRA_DES.PutData();
                    }
                    //COMLINE.Value = "$cmd/transfer_f020_f027_f028 " + fleF020_DOCTOR_MSTR.GetStringValue("DOC_NBR") + " " + fleF020_DOCTOR_EXTRA.GetStringValue("REPLACED_BY_DOC_NBR");
                    //m_blnCommandOK = RunCommand(COMLINE.Value);
                    object[] arrPS = { fleF020_DOCTOR_MSTR.GetStringValue("DOC_NBR"),
                        fleF020_DOCTOR_EXTRA.GetStringValue("REPLACED_BY_DOC_NBR") };
                    string ps = System.Configuration.ConfigurationManager.AppSettings["JobScriptPath"] + @"transfer_f020_f027_f028.ps1";
                    string psServer = System.Configuration.ConfigurationManager.AppSettings["psServer"].ToString();
                    PowerShellEngine pse = new PowerShellEngine();
                    pse.ExecuteScriptFile(ps, arrPS, psServer);
                    pse.Dispose();
                    // TODO: Check source code.  Manual process may be required.
                    Information(QDesign.NULL("F020/f027/f028  will be transferred to replaced by doctor records "));
                    // TODO: May need to fix manually
                }
                if (QDesign.NULL(X_DOC_OSCAR_REPLACED.Value) == QDesign.NULL("Y"))
                {
        ...
        private void fldF020_DOCTOR_MSTR_DOC_DEPT_Process()
        {
        ...
        protected override bool DetailFind()
        {
            ...
            try
            {
                ...
                {
                    fleF020C_DOC_CLINIC_NEXT_BATCH_NBR.GetData(GetDataOptions.CreateSubSelect | GetDataOptions.IsOptional);
                }
        ...
        //# Precompiler Ver.: 1.0.6353.18277  Generated on: 5/29/2017 10:10:39 AM
        //#-----------------------------------------
        ...
        private void DsrDesigner_LOCA_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                RunScreen(new Billing_M020_LOC(), RunScreenModes.Same, ref arrRunscreen);
