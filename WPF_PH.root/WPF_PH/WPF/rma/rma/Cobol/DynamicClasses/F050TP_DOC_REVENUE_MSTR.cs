...
namespace RmaDAL
{
    public partial class F050TP_DOC_REVENUE_MSTR : BaseTable
    {
        #region Retrieve

        public ObservableCollection<F050TP_DOC_REVENUE_MSTR> Collection( Guid? rowid,
                                                            ...
                                                            int skip)
        {
            ...
            if (replaceSearch)
            {
                Reader = CoreReader("[INDEXED].[sp_F050TP_DOC_REVENUE_MSTR_RecordCount]", parameters);
				if (Reader.Read())
					TotalItemCount = (int)Reader[0];
				if (TotalItemCount == 0)
				{
					return new ObservableCollection<F050TP_DOC_REVENUE_MSTR>();
				}

            }

            Reader = CoreReader("[INDEXED].[sp_F050TP_DOC_REVENUE_MSTR_Search]", parameters);
            var collection = new ObservableCollection<F050TP_DOC_REVENUE_MSTR>();

            while (Reader.Read())
            {
                collection.Add(new F050TP_DOC_REVENUE_MSTR
                {
        ...
        public F050TP_DOC_REVENUE_MSTR Class()
        {
        ...
        public ObservableCollection<F050TP_DOC_REVENUE_MSTR> Collection(ObservableCollection<F050TP_DOC_REVENUE_MSTR>
                                                               f050tpDocRevenueMstr = null)
        {
            if (IsSameSearch() && f050tpDocRevenueMstr != null)
            {
                return f050tpDocRevenueMstr;
            }
			...
			Reader = CoreReader("[INDEXED].[sp_F050TP_DOC_REVENUE_MSTR_Match]", parameters);
            var collection = new ObservableCollection<F050TP_DOC_REVENUE_MSTR>();
...
;
        ...
        public bool Delete()
        {
			...
			RowsAffected = CoreExecuteNonQuery("[INDEXED].[sp_F050TP_DOC_REVENUE_MSTR_DeleteRow]", parameters);
		...
		{
		    int RowsAffected = 0;
		    RowsAffected = CoreExecuteNonQuery("[INDEXED].[sp_F050TP_DOC_REVENUE_MSTR_Purge]");
		    CloseConnection();
		    return true;
		}
        ...
        public void Submit()
        {
            ...
            switch (RecordState)
            {
                ...
                case State.Added:
					...
					Reader = CoreReader("[INDEXED].[sp_F050TP_DOC_REVENUE_MSTR_Insert]", parameters);
					if (Reader.Read())
					{
						RowCheckSum = Convert.ToInt32(Reader["RowCheckSum"]);
						ROWID = (Guid) Reader["ROWID"];
                ...
                case State.Modified:
					...
					Reader = CoreReader("[INDEXED].[sp_F050TP_DOC_REVENUE_MSTR_Update]", parameters);
					if (Reader.Read())
					{
						RowCheckSum = Convert.ToInt32(Reader["RowCheckSum"]);
						ROWID = (Guid) Reader["ROWID"];
