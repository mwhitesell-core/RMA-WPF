...
namespace RmaDAL
{
    public partial class F096_RA_REJECT_CODE : BaseTable
    {
        #region Retrieve

        public ObservableCollection<F096_RA_REJECT_CODE> Collection( Guid? rowid,
															string rat_code,
                                                            ...
                                                            int skip)
        {
            ...
            if (replaceSearch)
            {
                Reader = CoreReader("[INDEXED].[sp_F096_RA_REJECT_CODE_RecordCount]", parameters);
				if (Reader.Read())
					TotalItemCount = (int)Reader[0];
				if (TotalItemCount == 0)
				{
					return new ObservableCollection<F096_RA_REJECT_CODE>();
				}

            }

            Reader = CoreReader("[INDEXED].[sp_F096_RA_REJECT_CODE_Search]", parameters);
            var collection = new ObservableCollection<F096_RA_REJECT_CODE>();

            while (Reader.Read())
            {
                collection.Add(new F096_RA_REJECT_CODE
                {
        ...
        public F096_RA_REJECT_CODE Class()
        {
        ...
        public ObservableCollection<F096_RA_REJECT_CODE> Collection(ObservableCollection<F096_RA_REJECT_CODE>
                                                               f096RaRejectCode = null)
        {
            if (IsSameSearch() && f096RaRejectCode != null)
            {
                return f096RaRejectCode;
            }
			...
			Reader = CoreReader("[INDEXED].[sp_F096_RA_REJECT_CODE_Match]", parameters);
            var collection = new ObservableCollection<F096_RA_REJECT_CODE>();
...
;
        ...
        public bool Delete()
        {
			...
			RowsAffected = CoreExecuteNonQuery("[INDEXED].[sp_F096_RA_REJECT_CODE_DeleteRow]", parameters);
		...
		{
		    int RowsAffected = 0;
		    RowsAffected = CoreExecuteNonQuery("[INDEXED].[sp_F096_RA_REJECT_CODE_Purge]");
		    CloseConnection();
		    return true;
		}
        ...
        public void Submit()
        {
            ...
            switch (RecordState)
            {
                ...
                case State.Added:
					...
					Reader = CoreReader("[INDEXED].[sp_F096_RA_REJECT_CODE_Insert]", parameters);
					if (Reader.Read())
					{
						RowCheckSum = Convert.ToInt32(Reader["RowCheckSum"]);
						ROWID = (Guid) Reader["ROWID"];
                ...
                case State.Modified:
					...
					Reader = CoreReader("[INDEXED].[sp_F096_RA_REJECT_CODE_Update]", parameters);
					if (Reader.Read())
					{
						RowCheckSum = Convert.ToInt32(Reader["RowCheckSum"]);
						ROWID = (Guid) Reader["ROWID"];
