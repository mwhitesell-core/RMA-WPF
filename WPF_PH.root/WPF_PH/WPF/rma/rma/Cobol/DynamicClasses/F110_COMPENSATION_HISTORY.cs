...
namespace RmaDAL
{
    public partial class F110_COMPENSATION_HISTORY : BaseTable
    {
        #region Retrieve

        public ObservableCollection<F110_COMPENSATION_HISTORY> Collection( Guid? rowid,
															string doc_nbr,
															decimal? ep_nbrmin,
															decimal? ep_nbrmax,
                                                            ...
                                                            int skip)
        {
            ...
            if (replaceSearch)
            {
                Reader = CoreReader("[INDEXED].[sp_F110_COMPENSATION_HISTORY_RecordCount]", parameters);
				if (Reader.Read())
					TotalItemCount = (int)Reader[0];
				if (TotalItemCount == 0)
				{
					return new ObservableCollection<F110_COMPENSATION_HISTORY>();
				}

            }

            Reader = CoreReader("[INDEXED].[sp_F110_COMPENSATION_HISTORY_Search]", parameters);
            var collection = new ObservableCollection<F110_COMPENSATION_HISTORY>();

            while (Reader.Read())
            {
                collection.Add(new F110_COMPENSATION_HISTORY
                {
        ...
        public F110_COMPENSATION_HISTORY Class()
        {
        ...
        public ObservableCollection<F110_COMPENSATION_HISTORY> Collection(ObservableCollection<F110_COMPENSATION_HISTORY>
                                                               f110CompensationHistory = null)
        {
            if (IsSameSearch() && f110CompensationHistory != null)
            {
                return f110CompensationHistory;
            }
			...
			Reader = CoreReader("[INDEXED].[sp_F110_COMPENSATION_HISTORY_Match]", parameters);
            var collection = new ObservableCollection<F110_COMPENSATION_HISTORY>();
...
;
        ...
        public bool Delete()
        {
			...
			RowsAffected = CoreExecuteNonQuery("[INDEXED].[sp_F110_COMPENSATION_HISTORY_DeleteRow]", parameters);
		...
		{
		    int RowsAffected = 0;
		    RowsAffected = CoreExecuteNonQuery("[INDEXED].[sp_F110_COMPENSATION_HISTORY_Purge]");
		    CloseConnection();
		    return true;
		}
        ...
        public void Submit()
        {
            ...
            switch (RecordState)
            {
                ...
                case State.Added:
					...
					Reader = CoreReader("[INDEXED].[sp_F110_COMPENSATION_HISTORY_Insert]", parameters);
					if (Reader.Read())
					{
						RowCheckSum = Convert.ToInt32(Reader["RowCheckSum"]);
						ROWID = (Guid) Reader["ROWID"];
                ...
                case State.Modified:
					...
					Reader = CoreReader("[INDEXED].[sp_F110_COMPENSATION_HISTORY_Update]", parameters);
					if (Reader.Read())
					{
						RowCheckSum = Convert.ToInt32(Reader["RowCheckSum"]);
						ROWID = (Guid) Reader["ROWID"];
