...
namespace RmaDAL
{
    public partial class F112_PYCDCEILINGS : BaseTable
    {
        #region Retrieve

        public ObservableCollection<F112_PYCDCEILINGS> Collection( Guid? rowid,
															string doc_nbr,
															decimal? ep_nbrmin,
															decimal? ep_nbrmax,
															decimal? factormin,
                                                            ...
                                                            int skip)
        {
            ...
            if (replaceSearch)
            {
                Reader = CoreReader("[INDEXED].[sp_F112_PYCDCEILINGS_RecordCount]", parameters);
				if (Reader.Read())
					TotalItemCount = (int)Reader[0];
				if (TotalItemCount == 0)
				{
					return new ObservableCollection<F112_PYCDCEILINGS>();
				}

            }

            Reader = CoreReader("[INDEXED].[sp_F112_PYCDCEILINGS_Search]", parameters);
            var collection = new ObservableCollection<F112_PYCDCEILINGS>();

            while (Reader.Read())
            {
                collection.Add(new F112_PYCDCEILINGS
                {
        ...
        public F112_PYCDCEILINGS Class()
        {
        ...
        public ObservableCollection<F112_PYCDCEILINGS> Collection(ObservableCollection<F112_PYCDCEILINGS>
                                                               f112Pycdceilings = null)
        {
            if (IsSameSearch() && f112Pycdceilings != null)
            {
                return f112Pycdceilings;
            }
			...
			Reader = CoreReader("[INDEXED].[sp_F112_PYCDCEILINGS_Match]", parameters);
            var collection = new ObservableCollection<F112_PYCDCEILINGS>();
...
;
        ...
        public bool Delete()
        {
			...
			RowsAffected = CoreExecuteNonQuery("[INDEXED].[sp_F112_PYCDCEILINGS_DeleteRow]", parameters);
		...
		{
		    int RowsAffected = 0;
		    RowsAffected = CoreExecuteNonQuery("[INDEXED].[sp_F112_PYCDCEILINGS_Purge]");
		    CloseConnection();
		    return true;
		}
        ...
        public void Submit()
        {
            ...
            switch (RecordState)
            {
                ...
                case State.Added:
					...
					Reader = CoreReader("[INDEXED].[sp_F112_PYCDCEILINGS_Insert]", parameters);
					if (Reader.Read())
					{
						RowCheckSum = Convert.ToInt32(Reader["RowCheckSum"]);
						ROWID = (Guid) Reader["ROWID"];
                ...
                case State.Modified:
					...
					Reader = CoreReader("[INDEXED].[sp_F112_PYCDCEILINGS_Update]", parameters);
					if (Reader.Read())
					{
						RowCheckSum = Convert.ToInt32(Reader["RowCheckSum"]);
						ROWID = (Guid) Reader["ROWID"];
