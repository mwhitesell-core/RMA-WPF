...
namespace RmaDAL
{
    public partial class U021A_EDT_1HT_FILE : BaseTable
    {
        #region Retrieve

        public ObservableCollection<U021A_EDT_1HT_FILE> Collection( Guid? rowid,
                                                            ...
                                                            int skip)
        {
            ...
            if (replaceSearch)
            {
                Reader = CoreReader("[SEQUENTIAL].[sp_U021A_EDT_1HT_FILE_RecordCount]", parameters);
				if (Reader.Read())
					TotalItemCount = (int)Reader[0];
				if (TotalItemCount == 0)
				{
					return new ObservableCollection<U021A_EDT_1HT_FILE>();
				}

            }

            Reader = CoreReader("[SEQUENTIAL].[sp_U021A_EDT_1HT_FILE_Search]", parameters);
            var collection = new ObservableCollection<U021A_EDT_1HT_FILE>();

            while (Reader.Read())
            {
                collection.Add(new U021A_EDT_1HT_FILE
                {
        ...
        public U021A_EDT_1HT_FILE Class()
        {
        ...
        public ObservableCollection<U021A_EDT_1HT_FILE> Collection(ObservableCollection<U021A_EDT_1HT_FILE>
                                                               u021aEdt1htFile = null)
        {
            if (IsSameSearch() && u021aEdt1htFile != null)
            {
                return u021aEdt1htFile;
            }
			...
			Reader = CoreReader("[SEQUENTIAL].[sp_U021A_EDT_1HT_FILE_Match]", parameters);
            var collection = new ObservableCollection<U021A_EDT_1HT_FILE>();
...
;
        ...
        public bool Delete()
        {
			...
			RowsAffected = CoreExecuteNonQuery("[SEQUENTIAL].[sp_U021A_EDT_1HT_FILE_DeleteRow]", parameters);
		...
		{
		    int RowsAffected = 0;
		    RowsAffected = CoreExecuteNonQuery("[SEQUENTIAL].[sp_U021A_EDT_1HT_FILE_Purge]");
		    CloseConnection();
		    return true;
		}
        ...
        public void Submit()
        {
            ...
            switch (RecordState)
            {
                ...
                case State.Added:
					...
					Reader = CoreReader("[SEQUENTIAL].[sp_U021A_EDT_1HT_FILE_Insert]", parameters);
					if (Reader.Read())
					{
						RowCheckSum = Convert.ToInt32(Reader["RowCheckSum"]);
						ROWID = (Guid) Reader["ROWID"];
                ...
                case State.Modified:
					...
					Reader = CoreReader("[SEQUENTIAL].[sp_U021A_EDT_1HT_FILE_Update]", parameters);
					if (Reader.Read())
					{
						RowCheckSum = Convert.ToInt32(Reader["RowCheckSum"]);
						ROWID = (Guid) Reader["ROWID"];
