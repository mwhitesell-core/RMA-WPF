...
namespace RmaDAL
{
    public partial class F119_DOCTOR_YTD : BaseTable
    {
        #region Retrieve

        public ObservableCollection<F119_DOCTOR_YTD> Collection( Guid? rowid,
															string doc_nbr,
                                                            ...
                                                            int skip)
        {
            ...
            if (replaceSearch)
            {
                Reader = CoreReader("[INDEXED].[sp_F119_DOCTOR_YTD_RecordCount]", parameters);
				if (Reader.Read())
					TotalItemCount = (int)Reader[0];
				if (TotalItemCount == 0)
				{
					return new ObservableCollection<F119_DOCTOR_YTD>();
				}

            }

            Reader = CoreReader("[INDEXED].[sp_F119_DOCTOR_YTD_Search]", parameters);
            var collection = new ObservableCollection<F119_DOCTOR_YTD>();

            while (Reader.Read())
            {
                collection.Add(new F119_DOCTOR_YTD
                {
					...
					_originalRec_1 = Reader["REC_1"].ToString(),
        ...
        public ObservableCollection<F119_DOCTOR_YTD> Collection(ObservableCollection<F119_DOCTOR_YTD>
                                                               f119DoctorYtd = null)
        {
            if (IsSameSearch() && f119DoctorYtd != null)
            {
                return f119DoctorYtd;
            }
            ...
            if (IsBlankSearch())
            {
            	...
            	var parameters = new SqlParameter[]
				{
					...
					new SqlParameter("REC_1",WhereRec_1),
			...
			Reader = CoreReader("[INDEXED].[sp_F119_DOCTOR_YTD_Match]", parameters);
            var collection = new ObservableCollection<F119_DOCTOR_YTD>();
...
;
		...
		public string WhereRec_1 { get; set; }
		private string _whereRec_1;
        ...
        public bool Delete()
        {
			...
			RowsAffected = CoreExecuteNonQuery("[INDEXED].[sp_F119_DOCTOR_YTD_DeleteRow]", parameters);
		...
		{
		    int RowsAffected = 0;
		    RowsAffected = CoreExecuteNonQuery("[INDEXED].[sp_F119_DOCTOR_YTD_Purge]");
		    CloseConnection();
		    return true;
		}
            ...
            switch (RecordState)
            {
                ...
                case State.Added:
					parameters = new SqlParameter[]
					{
						...
						new SqlParameter("REC_1", SqlNull(REC_1)),
					...
					Reader = CoreReader("[INDEXED].[sp_F119_DOCTOR_YTD_Insert]", parameters);
					if (Reader.Read())
					{
						RowCheckSum = Convert.ToInt32(Reader["RowCheckSum"]);
						ROWID = (Guid) Reader["ROWID"];
                ...
                case State.Modified:
					...
					Reader = CoreReader("[INDEXED].[sp_F119_DOCTOR_YTD_Update]", parameters);
					if (Reader.Read())
					{
						RowCheckSum = Convert.ToInt32(Reader["RowCheckSum"]);
						ROWID = (Guid) Reader["ROWID"];
