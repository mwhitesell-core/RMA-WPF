...
namespace RmaDAL
{
    public partial class F093_OHIP_ERROR_MSG_MSTR : BaseTable
    {
        #region Retrieve

        public ObservableCollection<F093_OHIP_ERROR_MSG_MSTR> Collection( Guid? rowid,
															string ohip_err_code,
															string ohip_err_description,
                                                            ...
                                                            int skip)
        {
            	var parameters = new SqlParameter[]
				{
					...
					new SqlParameter("OHIP_ERR_DESCRIPTION",ohip_err_description),
            ...
            if (replaceSearch)
            {
                Reader = CoreReader("[INDEXED].[sp_F093_OHIP_ERROR_MSG_MSTR_RecordCount]", parameters);
				if (Reader.Read())
					TotalItemCount = (int)Reader[0];
				if (TotalItemCount == 0)
				{
					return new ObservableCollection<F093_OHIP_ERROR_MSG_MSTR>();
				}

            }

            Reader = CoreReader("[INDEXED].[sp_F093_OHIP_ERROR_MSG_MSTR_Search]", parameters);
            var collection = new ObservableCollection<F093_OHIP_ERROR_MSG_MSTR>();

            while (Reader.Read())
            {
                collection.Add(new F093_OHIP_ERROR_MSG_MSTR
                {
					RowCheckSum = Convert.ToInt32(Reader["RowCheckSum"]),
					ROWID = (Guid)Reader["ROWID"],
					OHIP_ERR_CODE = Reader["OHIP_ERR_CODE"].ToString(),
					OHIP_ERR_DESCRIPTION = Reader["OHIP_ERR_DESCRIPTION"].ToString(),
					...
					_originalOhip_err_description = Reader["OHIP_ERR_DESCRIPTION"].ToString(),
        ...
        public F093_OHIP_ERROR_MSG_MSTR Class()
        {
        ...
        public ObservableCollection<F093_OHIP_ERROR_MSG_MSTR> Collection(ObservableCollection<F093_OHIP_ERROR_MSG_MSTR>
                                                               f093OhipErrorMsgMstr = null)
        {
            if (IsSameSearch() && f093OhipErrorMsgMstr != null)
            {
                return f093OhipErrorMsgMstr;
            }
            ...
            if (IsBlankSearch())
            {
            	...
            	var parameters = new SqlParameter[]
				{
					...
					new SqlParameter("OHIP_ERR_DESCRIPTION",WhereOhip_err_description),
			...
			Reader = CoreReader("[INDEXED].[sp_F093_OHIP_ERROR_MSG_MSTR_Match]", parameters);
            var collection = new ObservableCollection<F093_OHIP_ERROR_MSG_MSTR>();
            ...
            while (Reader.Read())
            {
                collection.Add(new F093_OHIP_ERROR_MSG_MSTR
                {
					...
					_whereOhip_err_description = WhereOhip_err_description,
        ...
        private bool IsBlankSearch()
        {
            if (WhereRowid == null 
				...
				&& WhereOhip_err_description == null 
        ...
        private bool IsSameSearch()
        {
           return WhereRowid ==  _whereRowid
				...
				&& WhereOhip_err_description ==  _whereOhip_err_description
...
;
		...
		private string _OHIP_ERR_DESCRIPTION;
		...
		public string OHIP_ERR_DESCRIPTION
		{
			get { return _OHIP_ERR_DESCRIPTION; }
			set
			{
				if (_OHIP_ERR_DESCRIPTION != value)
				{
					_OHIP_ERR_DESCRIPTION = value;
					ChangeState();
				}
			}
		}
		...
		public string WhereOhip_err_description { get; set; }
		private string _whereOhip_err_description;
		...
		private string _originalOhip_err_description;
        ...
        public bool Undo()
        {
			...
			OHIP_ERR_DESCRIPTION = _originalOhip_err_description;
        ...
        public bool Delete()
        {
			...
			RowsAffected = CoreExecuteNonQuery("[INDEXED].[sp_F093_OHIP_ERROR_MSG_MSTR_DeleteRow]", parameters);
		...
		{
		    int RowsAffected = 0;
		    RowsAffected = CoreExecuteNonQuery("[INDEXED].[sp_F093_OHIP_ERROR_MSG_MSTR_Purge]");
		    CloseConnection();
		    return true;
		}
            ...
            switch (RecordState)
            {
                ...
                case State.Added:
					parameters = new SqlParameter[]
					{
						...
						new SqlParameter("OHIP_ERR_DESCRIPTION", SqlNull(OHIP_ERR_DESCRIPTION)),
					...
					Reader = CoreReader("[INDEXED].[sp_F093_OHIP_ERROR_MSG_MSTR_Insert]", parameters);
					if (Reader.Read())
					{
						RowCheckSum = Convert.ToInt32(Reader["RowCheckSum"]);
						ROWID = (Guid) Reader["ROWID"];
                ...
                case State.Modified:
					...
					Reader = CoreReader("[INDEXED].[sp_F093_OHIP_ERROR_MSG_MSTR_Update]", parameters);
					if (Reader.Read())
					{
						RowCheckSum = Convert.ToInt32(Reader["RowCheckSum"]);
						ROWID = (Guid) Reader["ROWID"];
