...
namespace RmaDAL
{
    public partial class AFP_A4T_FILE : BaseTable
    {
        #region Retrieve

        public ObservableCollection<AFP_A4T_FILE> Collection( Guid? rowid,
                                                            ...
                                                            int skip)
        {
            ...
            if (replaceSearch)
            {
                Reader = CoreReader("[SEQUENTIAL].[sp_AFP_A4T_FILE_RecordCount]", parameters);
				if (Reader.Read())
					TotalItemCount = (int)Reader[0];
				if (TotalItemCount == 0)
				{
					return new ObservableCollection<AFP_A4T_FILE>();
				}

            }

            Reader = CoreReader("[SEQUENTIAL].[sp_AFP_A4T_FILE_Search]", parameters);
            var collection = new ObservableCollection<AFP_A4T_FILE>();

            while (Reader.Read())
            {
                collection.Add(new AFP_A4T_FILE
                {
        ...
        public ObservableCollection<AFP_A4T_FILE> Collection(ObservableCollection<AFP_A4T_FILE>
                                                               afpA4tFile = null)
        {
            if (IsSameSearch() && afpA4tFile != null)
            {
                return afpA4tFile;
            }
			...
			Reader = CoreReader("[SEQUENTIAL].[sp_AFP_A4T_FILE_Match]", parameters);
            var collection = new ObservableCollection<AFP_A4T_FILE>();
...
;
        ...
        public bool Delete()
        {
			...
			RowsAffected = CoreExecuteNonQuery("[SEQUENTIAL].[sp_AFP_A4T_FILE_DeleteRow]", parameters);
		...
		{
		    int RowsAffected = 0;
		    RowsAffected = CoreExecuteNonQuery("[SEQUENTIAL].[sp_AFP_A4T_FILE_Purge]");
		    CloseConnection();
		    return true;
		}
        ...
        public void Submit()
        {
            ...
            switch (RecordState)
            {
                ...
                case State.Added:
					...
					Reader = CoreReader("[SEQUENTIAL].[sp_AFP_A4T_FILE_Insert]", parameters);
					if (Reader.Read())
					{
						RowCheckSum = Convert.ToInt32(Reader["RowCheckSum"]);
						ROWID = (Guid) Reader["ROWID"];
                ...
                case State.Modified:
					...
					Reader = CoreReader("[SEQUENTIAL].[sp_AFP_A4T_FILE_Update]", parameters);
					if (Reader.Read())
					{
						RowCheckSum = Convert.ToInt32(Reader["RowCheckSum"]);
						ROWID = (Guid) Reader["ROWID"];
