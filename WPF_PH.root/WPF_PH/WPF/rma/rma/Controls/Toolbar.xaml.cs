...
namespace rma.Controls
{
    ...
    public partial class Toolbar : UserControl
    {
        ...
        {
            var helptext = ApplicationState.Current.CorePage.GetHelpScreenItem(ApplicationState.Current.Language, ApplicationState.Current.CorePage.FormName);
            var text = "";

            if (helptext == null)
                text = "No Help Availible.";

            var helpWindow = new HelpWindow("Screen Help: " + Title.Text, text) { Owner = Application.Current.MainWindow };
        ...
        public void ToolBarKeyDown(object sender, KeyEventArgs e)
        {
            ...
            else if (CloseButton.IsEnabled && e.Key == Key.C && (Keyboard.IsKeyDown(Key.LeftCtrl) || Keyboard.IsKeyDown(Key.RightCtrl)))
            {
