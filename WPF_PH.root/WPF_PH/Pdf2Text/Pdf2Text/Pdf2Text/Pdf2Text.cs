...
namespace Pdf2Text
{
    public static class Pdf2Text
    {
        private static string PAGE_CHAR = ""; // Linefeed character.
        private static string textFileName = string.Empty;
        private static string configFile = string.Empty;
        private static bool keepCorePdf = true;
        private static bool firstPageWritten = false;
        private static bool replaceWithFormFeeds = false;
        private static int pgcnter = 0;

        public static void Convert(string filePath)
        {
            Convert(filePath, null);
        }

        private static void ReadConfigurationSettings()
        {
            if (ConfigurationManager.AppSettings["KeepCorePDF"] == null)
                AddConfigurationSetting();

            try
            {
                keepCorePdf = ConfigurationManager.AppSettings["KeepCorePDF"].ToString().ToLower() == "true" ? true : false;
            }
            catch
            {
                // Write a log file if we have issues reading the KeepCorePDF entry.
                // If we get here, then we couldn't add the missing entry or create the App.Config file.
                using (StreamWriter sw = new StreamWriter("Pdf2Text.Log", false, System.Text.Encoding.UTF8))
                {
                    sw.Write("Error reading Config setting KeepCorePDF");
                }
            }
        }

        private static void AddConfigurationSetting()
        {
            // Get the active config file name.
            ...
            {
                configFile = AppDomain.CurrentDomain.SetupInformation.ApplicationBase + AppDomain.CurrentDomain.SetupInformation.ApplicationName + ".config";

                sb.Append("<?xml version=\"1.0\" encoding=\"utf-8\" ?>");
                sb.Append("<configuration>");
                sb.Append("  <appSettings>");
                sb.Append("    <add key=\"KeepCorePDF\" value=\"False\" />");
                sb.Append("  </appSettings>");
                sb.Append("</configuration>");

                using (StreamWriter sw = new StreamWriter(configFile, false, System.Text.Encoding.UTF8))
                {
            ...
            else
            {
                ...
                if (sb.ToString().Contains("<appSettings>"))
                    sb.Replace("<appSettings>", "<appSettings>\r\n    <add key=\"KeepCorePDF\" value=\"True\" />");
                else if (sb.ToString().Contains("<configuration>"))
                    sb.Replace("<configuration>", "<configuration>\r\n  <appSettings>\r\n    <add key=\"KeepCorePDF\" value=\"True\" />\r\n  </appSettings>");
                else if (sb.Length == 0){
                    sb.Append("<?xml version=\"1.0\" encoding=\"utf-8\" ?>");
                    sb.Append("<configuration>");
                    sb.Append("  <appSettings>");
            ...
            // Refresh the configuration manager section.
            ConfigurationManager.RefreshSection("appSettings");
        }

        public static void Convert(string filePath, string outputFileName)
        {
            ...
            try
            {
                ...
                if (outputFileName != null)
                    ...
                    textFileName = System.IO.Path.GetFileNameWithoutExtension(fileName);

                if (System.IO.Path.GetFileNameWithoutExtension(fileName).EndsWith(".FF") || System.IO.Path.GetFileNameWithoutExtension(fileName).EndsWith(".ff"))
                {
                    replaceWithFormFeeds = true;
                    textFileName = textFileName.Substring(0, textFileName.Length - 3);
                }
                else if (System.IO.Path.GetFileNameWithoutExtension(fileName).EndsWith(".RF") || System.IO.Path.GetFileNameWithoutExtension(fileName).EndsWith(".rf"))
                {
                    PAGE_CHAR = string.Empty;
                    textFileName = textFileName.Substring(0, textFileName.Length - 3);
                }

                if (replaceWithFormFeeds)
                    GenerateTextFileUsingFormFeedMethod(fileName);
                else
                    GenerateTextFile(fileName);

                // If keepCorePdf is false, then delete the PDF file.
                if (!keepCorePdf)
                    File.Delete(filePath);
                else if (replaceWithFormFeeds) // We have a file that has .FF in name, so rename file.
                {
                    if (File.Exists(filePath.Replace(".FF.", ".").Replace(".ff.", ".")))
                    {
                        File.Delete(filePath.Replace(".FF.", ".").Replace(".ff.", "."));
                    }
                    File.Move(filePath, filePath.Replace(".FF.", ".").Replace(".ff.", "."));
                }
                else if (PAGE_CHAR == string.Empty)
                { // We have a file that has .RF in name, so rename file.
                    if (File.Exists(filePath.Replace(".RF.", ".").Replace(".rf.", ".")))
                    {
                        File.Delete(filePath.Replace(".RF.", ".").Replace(".rf.", "."));
                    }
                    File.Move(filePath, filePath.Replace(".RF.", ".").Replace(".rf.", "."));
                }
            }
            catch (Exception ex)
            {
        ...
        public static void GenerateTextFile(string path)
        {
            ...
            string outputFile = System.IO.Path.Combine(System.IO.Path.GetDirectoryName(path), textFileName + ".txt");
                    ...
                    foreach (var p in t.myPoints)
                    {
                        ...
                        else
                        {
                            ...
                            if (existing.Count() == 0)
                                ...
                                err = p.Text; // Sometimes iTextSharp returns another value at same position which so far appears incorrect...
                        }
                    }
        ...
        {
            StringBuilder text = new StringBuilder(firstPageWritten ? PAGE_CHAR : string.Empty);
        ...
        // Generate the .txt file.
        public static void GenerateTextFileUsingFormFeedMethod(string path)
        {
            List<TextElement> data = null;
            string outputFile = System.IO.Path.Combine(System.IO.Path.GetDirectoryName(path), textFileName + ".txt");
            ...
            using (var r = new PdfReader(path))
            {
                for (int i = 1; i <= r.NumberOfPages; i++)
                {
                    ...
                    foreach (var p in t.myPoints)
                    {
                        ...
                        //if (p.Text.Contains("Special Weekend Coverage"))
                        //    row = row + 1 - 1;
                        ...
                        else
                        {
                            ...
                            if (existing.Count() == 0)
                            {
                                if (p.Text.EndsWith("~~"))
                                {
                                    ...
                                    data.Add(new TextElement(row, column, p.Text.Replace("~~", "")));
                                    var formFeedRowData = data.Where(x => x.Row == formFeedRow && x.Column == 1);
                                    if (formFeedRowData.Count() == 0)
                                        data.Add(new TextElement(formFeedRow, 1, PAGE_CHAR));
                                    else
                                        formFeedRowData.First().Text = PAGE_CHAR + formFeedRowData.First().Text;
                                }
                                else
                                {
                                    if (p.Text.IndexOf("\f") > -1)
                                        data.Add(new TextElement(row, column, p.Text.Replace("\f", "")));
                                    else
                                        data.Add(new TextElement(row, column, p.Text));
                                }
                            }
                            ...
                            {
                                if (formFeedRow == row)
                                    existing.First().Text = existing.First().Text + p.Text;
                                else
                                    err = p.Text; // Sometimes iTextSharp returns another value at same position which so far appears incorrect...
                            }
                        }
                    ...
                    // Now write out each page.
                    //if (formFeedRow > -1)
                        WritePageUsingFormFeedMethod(data, outputFile, i == 1);
        ...
        // Loop through controls and write out page to text file.
        private static void WritePageUsingFormFeedMethod(List<TextElement> data, string outputFile, bool isFirstWrite)
        {
            StringBuilder text = new StringBuilder(firstPageWritten && !replaceWithFormFeeds ? PAGE_CHAR : string.Empty);
            StringBuilder line = new StringBuilder(string.Empty);
            ...
            // Append the last line.
            //if (replaceWithFormFeeds)
            //    text.Append(line.ToString()); // Since we're adding the FF character on the next line, we want the next line to continue on same line.
            //else
                text.AppendLine(line.ToString());
