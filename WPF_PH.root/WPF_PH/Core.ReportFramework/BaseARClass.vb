...
#Region " Variables "
        ...
        Public m_cnnQUERY As IfxConnection
        Public m_cnnTRANS As IfxConnection
        Public m_trnTRANS As IfxTransaction
        ...
        Private m_sftSubFileType As Core.Framework.Core.Framework.SubFileType = Nothing
        ...
        Public m_objSession As Web.SessionState.HttpSessionState
...
#Region " Properies "
        ...
        ''' </history>
        ''' --- End of Comments ----------------------------------------------------
        Public ReadOnly Property Session() As Web.SessionState.HttpSessionState
            Get
                Return m_objSession
            End Get
        End Property
        ...
        Public Property DebugReport(Optional ByVal strLogPath As String = cDEFAULT_LOG_PATH + cDEFAULT_LOG) As Boolean
            Get
                Return m_blnDebug
            End Get
            Set(ByVal value As Boolean)
                ...
                If value Then
                    If strReportLogFile.Trim = "" Then
                        rptLog = New LogManager(strLogPath, 2, ReportName)      ' cREPORT_LOG
                    Else
                        rptLog = New LogManager(strReportLogFile, 2, ReportName)
                    End If
...
#Region " Public "
        ...
        Public Sub InitializeReport(ByVal strReportname As String)
            ...
            Try
                ...
                Try
                    m_cnnTRANS = New IfxConnection(strConnString)
                    m_cnnTRANS.Open()

                    m_trnTRANS = m_cnnTRANS.BeginTransaction

                    m_cnnQUERY = New IfxConnection(strConnString)
                    m_cnnQUERY.Open()

                Catch ifxEx As IfxException
                    If DebugReport Then
                        rptLog.WriteToLogFile("ERROR: IfxException..." + ifxEx.Errors.ToString)
                    End If
        ...
        Public Function GetDataBaseDataTable() As DataTable
            ...
            Try
                rtdattable = InformixHelper.ExecuteDataTable(m_trnTRANS, CommandType.Text, sbSQL.ToString)
                Return rtdattable
            Catch ex As Exception
                Return New DataTable
            Finally
                ...
                If blnTableExists OrElse blnSessionTableExists Then
                    ...
                    Try
                        If sbSQL.ToString.Trim <> String.Empty Then
                            InformixHelper.ExecuteNonQuery(m_trnTRANS, CommandType.Text, sbSQL.ToString)
                        End If
                    ...
                    Try
                        ...
                        If m_blnSubFile OrElse m_blnTempFile Then
                            If m_sftSubFileType = SubFileType.Keep And Not m_blnTempFile Then
                                ...
                                InformixHelper.ExecuteNonQuery(m_trnTRANS, CommandType.Text, CreateTableSQL(sbCreateTableSQL, False))
                            Else
                        ...
                        Else
                            ...
                            InformixHelper.ExecuteNonQuery(m_trnTRANS, CommandType.Text, CreateTableSQL(sbCreateTableSQL, True))
                        End If
            ...
            Try
                If blnTemp Then
                    If blnTableCreated Then
                        ...
                        'ReportData = InformixHelper.ExecuteReader(m_trnTRANS, CommandType.Text, GetDataSQL(ReportName, Sort, blnTemp))
                    Else
